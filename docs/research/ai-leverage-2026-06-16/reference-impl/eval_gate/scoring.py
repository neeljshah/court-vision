...
Self-contained reference implementation (numpy + stdlib only). In production,
reuse kernel.validation.proof_metrics for brier/devig2/ece per the blueprint;
this standalone version exists so the gate's math is runnable + tested anywhere.
...
def ece(p: ArrayLike, y: ArrayLike, bins: int = 10) -> float:
    ...
    Never optimize ECE directly -- predicting 0.5 everywhere drives ECE toward
    zero with zero value. Always read alongside sharpness() and resolution().
    """
    p, y = _arr(p), _arr(y)
