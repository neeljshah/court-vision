...
| Mechanism ledger export | Every named mechanism across the 4-sport validation ledgers surfaced row-level with its recorded verdict, effect, corpus, and evidence pointer (12 raw verdict values normalized to 3 buckets) | `analytics_showcase/out/mechanism_ledger_export.json` | `docs/img/mechanism_ledger_export.png` | DESCRIPTIVE / measurement-hygiene, `edge_claimed: false`. 287 rows: confirmed 130 / null 126 / not_testable 31. By sport: NBA 45/33/7 (85), MLB 32/46/8 (86), soccer 29/27/8 (64), tennis 24/20/8 (52). Row-level companion to the aggregate `mechanism_survival` rates (section 8) -- no overlap. Reads canonical `domains/*/knowledge/validation_ledger.jsonl` only. A NULL/REJECT/NOT_TESTABLE is honest market-efficiency evidence, not a failure. `as_of` = recorded `run_ts` (null for 142/287 pre-stamping runs). |
...
## 10. True-intelligence wave (counterfactual - context-conditioned - microstructure - forward-graded - cross-sport)

Ten analytics that ask *why* and *what-if*, not just *how well calibrated*. Every one carries
`edge_claimed: false`; roster-confounded and underpowered reads are labelled as such, and the
honest `not_buildable` verdicts sit in the same table as the wins. Full narrative with charts:
[evidence/true-intelligence](evidence/true-intelligence.md).

| Analytic | What it measures | Artifact | Chart | Honest caveat |
|---|---|---|---|---|
| Counterfactual star removal | Pregame win-prob points each team's top on/off player is worth: subtract their on/off net-rating from team Elo, re-price through the repo Elo->prob curve | `analytics_showcase/out/cf_star_removal.json` | `docs/img/cf_star_removal.png` | CEILING, DESCRIPTIVE_ONLY. 30/30 teams. on/off net-rating is a **roster-confounded** full-lineup swing, not marginal individual value; net-rating->Elo is a single frozen (un-fit) constant. Jokic tops at +0.582 win-prob pts vs a 1500 opponent on a neutral floor -- not a real matchup, not causal. |
| Counterfactual pace variance | How possession count shifts underdog upset probability for 60/70/80% favorites across pace 92-104, via a normal-margin possession model parameterized from real NBA pace + margin data | `analytics_showcase/out/cf_pace_variance.json` | `docs/img/cf_pace_variance.png` | COUNTERFACTUAL/CALIBRATION, edge_claimed=false. Pace is a **modest** lever: a 70% favorite's upset prob moves only 29.6% (pace 104) -> 30.7% (pace 92). MC discrete-possession sim matched the closed-form within 0.0042. v_pp anchor slightly overstates within-game variance (cancels in the headline). |
| Context player splits | Top-50 minute players' TS% split by opponent defensive tier / home-away / rest, with a context-sensitivity score (mean abs TS% delta) | `analytics_showcase/out/ctx_player_splits.json` | `docs/img/ctx_player_splits.png` | DESCRIPTIVE_ONLY, 77,744 player-game rows, 3 seasons. Cells with n<10 marked insufficient and dropped; players with <2 usable dimensions score null. Most context-sensitive: Payton Pritchard (0.0441). Descriptive split, not a matchup projection. |
| Context team states | NBA team perf by game state: front-runner (2H margin leading at half) vs comeback (trailing), per-team quadrant with n-per-split floor masking + league mean-reversion slope | `analytics_showcase/out/ctx_team_states.json` | `docs/img/ctx_team_states.png` | Per-team quadrant **not_buildable**: 0/26 teams meet the n>=2-per-split floor from this 26-game corpus (masked, not faked). League headline instead: 2H-on-half slope=0.2885 (pearson r=0.2266, n=52 team-games) -- leads mildly persist, do not strongly revert. |
| Microstructure absorption | Pregame market microstructure from our own scraped line history: inter-update cadence, move magnitude by time-to-start bucket, and final-hour movement share per sport | `analytics_showcase/out/micro_absorption.json` | `docs/img/micro_absorption.png` | DESCRIPTIVE market science, edge_claimed=false. NBA final-hour movement share 0.3247 (n=182,054 move pairs); MLB 0.1813 (n=1,307,128). kbo/npb insufficient_data (below MIN_MOVES=20). A cadence/dispersion exhibit, not a signal. |
| Microstructure closing decay | Brier/log-loss of consensus devigged market P(home) at T-24h/T-6h/T-1h/close, paired within-game | `analytics_showcase/out/micro_closing_decay.json` | `docs/img/micro_closing_decay.png` | Single capture window, provisional. WNBA paired T-6h 0.2356 -> close 0.2347 (delta=+0.0009, n=25 shared games); soccer_intl paired T-24h 0.1355 -> close 0.1568 (delta=-0.0213, n=7) and **UNDERPOWERED**; nba/mlb/soccer/tennis/kbo/npb **not_joinable** (no line_history<->outcome join). |
| Forward claim scoreboard | Post-publication track record of every claim family (sport x hypothesis) from the 4 validation ledgers: verdict history, current status, flipped-verdict flag, nulls-per-confirm ratio | `analytics_showcase/out/fwd_claim_scoreboard.json` | `docs/img/fwd_claim_scoreboard.png` | The product **is** the self-grading. 259 families / 287 ledger rows: 121 verified vs 134 null-or-worse (99 null, 28 not_testable, 7 retracted, 4 provisional); honest nulls-per-confirm ratio 1.107; 5 families flipped verdict over time. A system that only ever confirmed would not be credible. |
| Cross-sport structure | One reliability-map methodology (favorite-longshot gap, tick-weighted market ECE/Brier, near-decided comeback rate) applied to all 4 sports | `analytics_showcase/out/xsport_structure.json` | `docs/img/xsport_structure.png` | Only NBA clears the n_games>=30 bucket floor: market ECE=0.0064 (n_games=1593), fav_gap +0.005 / dog_gap -0.001 (favorite-longshot-consistent), comeback rate 0.0146. MLB / soccer / tennis honestly **not_buildable** (no qualifying bucket / no map), never guessed. |
| Why-attribution | In-game win-prob moves attributed to adjacent-time state transitions (calibrated win-prob delta = mean_y(dest)-mean_y(src) from the state-conditioned buckets) | `analytics_showcase/out/why_attribution.json` | `docs/img/why_attribution.png` | Only mlb + soccer_intl have the buckets; **NBA skipped** (no qualifying transitions). Largest calibrated drop: soccer_intl 15-30min .4-.6 -> 30-45min .2-.4 = -0.7683 (min support n=120). Decomposition of moves that already happened, not a predictor. |
| Context lineup proxy | Team win rate with a player active (played) vs missed inside their own tenure window; Wald 95% CI on the difference. RAPM-free proxy | `analytics_showcase/out/ctx_lineup_proxy.json` | `docs/img/ctx_lineup_proxy.png` | DESCRIPTIVE_ONLY, 408 qualified players (2025-26). WITH/WITHOUT = **roster confound** (injuries cluster, rest games, blowout pulls, different replacement lineup -- none controlled). Top proxy help Brandon Miller +0.4543; CI (+/-0.2159) does not exclude a much smaller effect. Not causal, not an edge. |

---

## Shareable chart pack

`analytics_showcase/share_chart.py` re-renders 54 of the analytics catalogued above into
brand-palette PNGs under `docs/img/share/` (ledger `out/share_ledger.json`, 54/54 present). It
computes **no new analytic** -- it is a presentation layer over the artifacts in this catalog and
inherits each one's caveat verbatim. Every rendered chart carries a
`source: <artifact> | as_of: <stamp> | edge_claimed: false` footer read from the source artifact's
own metadata, so a chart can never be shared without its provenance and the not-an-edge disclaimer.
