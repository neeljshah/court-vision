...
|---|---|---|---|
| StatsBomb events | `data/cache/statsbomb/events/<mid>.json` | 400 matches x ~3,490 events | 2015-2021 open data |
...
| Referee card/foul profiles (verified claims) | `data/domains/soccer/referee_card_foul_profiles.parquet` + `data/cache/intel_claims/soccer_referee_card_foul_profiles*` | 2,736 claims, 2,736/2,736 verified | 2015-2026, per-year + per-league (E0/E1) |
| Compiled profiles | `data/cache/profiles/soccer_team_profiles.parquet` | 2,717 | rolling |
| Market | `data/cache/line_history/soccer/` | 5,940 rows; in-play tick history is an empty stub | 2026-06/07 through 07-08 |

Biggest known gap: event data (the only corpus with possession-level detail)
covers only 400 StatsBomb matches ending 2021 -- none of the 25,834-match
...
## Attribute catalog (7 attributes, `domains/soccer/profiles/attribute_registry.py`, team-only)

`counter_threat` (counter-attack goal threat, weighted by danger,
**VALIDATED_MECHANISM**), `buildup_quality` (regular-play attacking output
per possession, **VALIDATED_CLAIM**), `set_piece_threat` (share of a team's
total xG from set-piece-derived possessions, **VALIDATED_CLAIM**),
...
never testable at all:

- **`press_resistance`** (possessions lost in a team's own defensive third)
  -- **BLOCKED**, registered but not built. StatsBomb's `location` field is
  pitch-absolute (0-120 x 0-80), not normalized to attacking direction, and
  attacking direction flips at half-time and period boundaries. No
  attacking-direction resolver exists anywhere in this codebase. A
  wrong-sided proxy would silently invert the attribute (a team's
  "own-third" losses would read as the opponent's), so this was BLOCKED
...
- `python -m scripts.platformkit.profiles.ask --list --sport soccer` (all 7 attributes)

## What would make this deeper

- An attacking-direction resolver for StatsBomb's pitch-absolute
  coordinates -- the single blocker on `press_resistance` and on any future
  own-third / final-third attribute.
- Extending event coverage past 2021 -- the 400-match StatsBomb open-data
  corpus is the only source with possession-level detail, and it stops
