...
2. `pip install --break-system-packages "fastapi>=0.110" "uvicorn[standard]>=0.27" "scikit-learn==1.8.0" "pyarrow>=14" xgboost` (pyarrow was ALSO wiped -- an import-only preflight passed 14/14 while every parquet read failed; PEP 668 refuses without the flag; the container is disposable). Import-check the paper-profile modules before booting.
3. Boot the keeper: `cd /workspace/nba-ai-system && CV_CAPTURE_POD=1 CV_MLB_BOOK_ARCHIVE_LIVE=1 nohup setsid /usr/local/bin/python -u -m supervisor --profile paper </dev/null >/workspace/paper.log 2>&1 &` -- NOT `python3` (same 3.12 today, but keep the path explicit). Expect 14 children in /proc within 80 s and 0 `restart #` lines.
...
7. Provision the factory's DATA sources (S78): `git archive` ships only the tracked tree, and every source the real screen predictor reads is under gitignored `data/` -- run `python scripts/platformkit/ops/factory_source_manifest.py --pod-path-only --check-pod --ship` from the repo root (read-only; it PRINTS the tar-over-ssh command for the MISSING set), and `pod_bootstrap_check.py --functional` fails its `factory_sources` probe until they are there.
