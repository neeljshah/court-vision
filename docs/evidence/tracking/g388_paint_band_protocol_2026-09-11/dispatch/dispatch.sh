...
INSTR="$WT/docs/evidence/tracking/g388_paint_band_protocol_2026-09-11/rater_instruction_${PASS}.md"
OUTDIR="$CACHE/out_${PASS}_${R}"
mkdir -p "$OUTDIR"
gate() {
  while true; do
...
for B in "$CACHE/batch_${PASS}_${R}"/batch_*.tsv; do
  ...
  gate
  LOG="C:/Users/neelj/AppData/Local/Temp/cx_g388_rater_${R}_${PASS}${NN}.log"
  ...
  python /c/Users/neelj/bin/hidden_launch2.py --log "$LOG" --cwd "$WT" --tag "g388_rater_${R}_${PASS}${NN}" \
