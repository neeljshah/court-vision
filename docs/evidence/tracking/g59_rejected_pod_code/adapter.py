...
class TennisAdapter:
    ...
    def __init__(self, detector: Optional[Detector] = None, corner_stability_px: float = 5.0,
        ...
        self._player_selector = PlayerSelector()
    ...
    def _reset_temporal_calibration(self) -> None:
        ...
        self._camera_lock.reset(); self._player_selector.reset(); self._last_fresh_corners = None
    ...
    def _track_ids(self, candidates: list[tuple[np.ndarray, np.ndarray]]) -> list[tuple[int, np.ndarray]]:
        if len(candidates) == 1:
            center, foot = candidates[0]
            if self._centroids:
                track_id = min(self._centroids, key=lambda key: np.linalg.norm(center - self._centroids[key]))
                self._centroids[track_id] = center
            else:
                track_id = self._track_id_base + 1
                self._centroids[track_id] = center
            return [(track_id, foot)]
    ...
    def detect_players(self, frame: np.ndarray, homography: np.ndarray) -> list[tuple[int, np.ndarray]]:
        ...
        for box in self.detector(frame):
            ...
            confidence = float(box[4]) if len(box) >= 5 else 1.0
            track_id = box[5] if len(box) >= 6 else None
            candidates.append(PlayerCandidate(center, foot, confidence, track_id))
        per_half = self._player_selector.select(candidates)
        self.last_non_players = self._player_selector.last_non_players
        if not per_half:
            return []
        return self._track_ids([(per_half[half].center, per_half[half].foot)
