GAP G78C | sport tennis | worktree a8 | log cx_g78c_resolve_uncertain  (CHUNK of G78 -- ONE CLIP ONLY)
...
YOUR CHUNK -- READ THIS FIRST. Attempt 1 of G78 tried all 109 uncertain rows in one session, ran
out, and honestly reported nothing rather than fabricating -- but it DID confirm the premise: the
retained 2x tiles expose additional balls (tennis_09 ordinals 12-14 among them). Bulk visual review
at zoom is simply too much for one run, so G78 is split by clip.
**YOUR CLIP IS `tennis__tennis_nyYk2nPZAwY_720p.mp4` AND NOTHING ELSE. It has 30 uncertain rows.**
WRITE YOUR RESULTS INCREMENTALLY to
docs/evidence/tracking/g65_ball_labels/resolved/tennis__tennis_nyYk2nPZAwY_720p.csv, flushing after every 10 rows.
This is the single most important instruction here: a G68 lane lost 300 completed labels by holding
them in context, and that is exactly how attempt 1 of this row ended with nothing.
Do NOT compute the pooled resolved fraction and do NOT re-derive the 78-vs-52 y-gate figure -- an
aggregation row does that once every chunk lands. Report only YOUR clip's counts.
