#!/bin/bash
# G399 blind real-paint dispatch. usage: dispatch.sh <rater> <codex_home> <set>
# set = fixture | real. Mechanics are the G396 real-pass mechanics verbatim.
R="$1"; HOME_DIR="$2"; SET="$3"
CACHE=/c/Users/neelj/AppData/Local/Temp/g399_work
...
EVID="$WT/docs/evidence/tracking/g399_qualified_paint_larger_sample_2026-09-11"
INSTR="$EVID/frozen_instructions/rater_instruction_real.md"
[ "$SET" = fixture ] && INSTR="$EVID/frozen_instructions/rater_instruction_smoke.md"
OUTDIR="$WT/.g399_out/${SET}_${R}"
mkdir -p "$OUTDIR"
gate() {
  while true; do
...
for B in "$CACHE/batch_${SET}_${R}"/batch_*.tsv; do
  ...
  gate
  LOG="C:/Users/neelj/AppData/Local/Temp/cx_g399_rater_${R}_${SET}${NN}.log"
  ...
  python /c/Users/neelj/bin/hidden_launch2.py --log "$LOG" --cwd "$WT" --tag "g399_rater_${R}_${SET}${NN}" \
