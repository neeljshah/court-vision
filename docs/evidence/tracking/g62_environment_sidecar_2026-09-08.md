...
- NEW GAP: the out-of-scope loose-file row is 001401 in the committed `census.csv` but 001402 when recomputed from candidate HEAD, because the newly tracked memo is not excluded; the 259-directory result is unchanged.
- NEW GAP: executable opt-out byte-parity coverage exists for `census_recomputable` only; `g310_instance_key` and `g330_run_attempt2` opt-out parity is inspection-only.
