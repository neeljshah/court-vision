...
CORRECTION: line 34, and the same numbers in the RESULTS_LEDGER row, now read `424` / `421` / `0 of 424` (was `423` / `420` / `0 of 423`). The candidate-state scope is 424 git-tracked `.csv` files under `docs/evidence/tracking/`, re-counted at landing: 423 at base `cfd07ea08` plus this row's own `census.csv`, which is itself the added non-ball file and so lands in the "neither" bucket. Both ball-table outcomes are unchanged and no counted row moved.
CORRECTION: line 32, and the same wording in the comment at `tests/platformkit/test_g320_ball_inferred_no_coord.py:86-88`, now read `reported separately while retained in inclusive ball_inferred` (was `folded into neither ball_detected nor ball_inferred`). Prose and comment only -- no assertion was changed; the assertions immediately below that comment already encode the inclusive meaning (`inferred_no_coord + inferred_with_coords == ball_inferred`).
DIGEST: the test-file sha256 on the line above was refreshed to the landed file after that comment correction and now reads `4ec9f398d020c07845eaf68bbefcd9e6dde506f51367974e1411e77d452f80b5` (was `f1c84216c1f4de1adfceb7e1346b0b7a3d92f56d3c84933ba5d1e582083922cf`); the other four digests reproduce unchanged.
NEW GAP: the exhaustive csv denominator is not snapshot-stable, because the census output itself becomes a newly tracked `.csv` inside the very tree it scans -- which is exactly the 423 to 424 shift corrected above. Archive the input manifest, or define the output exclusion (`g320_ball_table_census.py:47-50,81-101`).
NEW GAP: the wall time recorded above ends at 07:20 CDT, later than the candidate commit time 07:14:43 -0500, so the reported interval is not commit-consistent and no rule ties the two together.
