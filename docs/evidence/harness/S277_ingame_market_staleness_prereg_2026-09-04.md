# S277 preregistration: NBA in-game market staleness
...
- The pre-score distribution has 463,656 ticks in scope after 1,593 named
  ...
  stale is age > p90; all remaining
  in-scope ticks are the named middle band. No tick's bin depends on outcome or
  either arm's loss.
...
- OOS scorer: `scripts.platformkit.eval_gate.cpcv_engine.cpcv_evaluate` with
  ...
  one-day calendar embargo. Each scored tick is represented as a state; the
  predictor only returns the precomputed frozen arm value and does not fit.
...
- Differential archive: write `S277_ingame_market_staleness_2026-09-04.md`,
  ...
  first-tick exclusion, a price-move pattern with a timestamp tie, and one
  archived-bin Brier reconstruction.

Seal SHA-256: fa3e0a0c500b14e8cd0e9549a9febb1268a5ba3045f949e0e780b7545b434393
