# S285 preregistration: event-proximity audit of the S265 static band
...
`ticks_since_last_score_change` using only the current tick and prior ticks:
compare its `(score_home, score_away)` with the immediately prior tick, record
zero when it changes, otherwise record the number of ticks since the last such
...
sample and fix the boundaries at its p50 and p90. The premise measurement
recorded p50=14 and p90=136: near-event is `<= 14`, settled is `> 136`, and
the named middle exclusion is `15..136`. Every held-out scored tick receives
exactly one of these labels. The boundaries are never re-fit after a metric is
seen.
...
SEAL_SHA256: 10b0e1f5990598d3f1f4fd4929245a795c623d12afad5d97a89066d1ba2a2a1a
