...
- Real-corpus counts can differ between this worktree and the landing main checkout; this worktree run has 43 screens and the known main count is unknown (not inspected from this worktree).
...
- Restored the legacy v3 memo and JSON to candidate-parent bytes; corrected output is written only to v5 paths.
