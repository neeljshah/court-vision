...
| SF-9 | 6 | Q1 ANSWER. The columns the CURRENT runner screens are exactly the video/tracking ones, and S16 extends that runner. `build_minutes_matrix` (:23) reads `data/nba/player_tracking_features_asof.parquet`, `player_load_state_asof.parquet` and `player_embeddings_asof.parquet` (25,699 rows each). NOT runtime-available, and a runtime-contract violation if screened as student features: `load_speed_elasticity`, `load_touch_elasticity`, `contest_rest_response`, `b2b_speed_drop` (`novel_metric_lift.CANDIDATE_METRICS`); `cum_distance_7d`, `cum_distance_14d`, `speed_decline_ratio` (`teacher_student_ab.LOAD_FEATURES`); `style_embedding_1..4`; and from the same tracking parquet `speed`, `distance`, `touches`, `passes`, `contestedFieldGoals*`, `uncontestedFieldGoals*`, `defendedAtRim*`, `reboundChances*` plus their `*_per36_l5/l10` rollups. Of `LOAD_FEATURES` only `days_rest` classifies RUNTIME; `b2b` classifies UNKNOWN. In the pregame catalogue the analogous training-only block is `asof_defender_rollup` (18 features: `def_switches_per_game_asof`, `def_matchup_min_asof`, `def_fg_pct_allowed_asof`, ...), which also classifies UNKNOWN, not TRAINING_ONLY. | HIGH | `foundry_runner.py:23-45`; the three parquet schemas read on disk; `classify_feature` run over each name. | Same as SF-8, plus: the grammar tags these families `runtime_available=False` and the runner routes them to the teacher lane (S04) only, never to a student screen. |
...
tagged `runtime_available=False` (teacher lane only) until SF-8's classifier gate is wired.
...
- `asof_defender_rollup` provenance was NOT traced to its builder; classifying it as
  tracking-derived rests on column semantics (`def_switches_per_game`, `def_matchup_min`), not on a
  read of the code that writes it.
