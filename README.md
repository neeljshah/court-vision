...
- **What's the moat?** Three layers, in order of defensibility. (1) **CV-pipeline depth + resilience** — 260 games end-to-end, 30 documented pipeline bugs patched in a single autonomous overnight loop, audit atlases that catch silent failure modes (Bug 33 ghost slots, Bug 6 jersey cross-team collisions, Bug 39 10-slot Hungarian-matching ceiling). Competitors buy Sportradar / Second Spectrum at six-to-seven-figure annual fees; we run on the same broadcast feed at ~$0.10–0.13 / game on a RunPod 3090. (2) **Intelligence layer between CV and the prop models** — 80+ derived artifacts (archetypes, schemes, lineup chemistry, matchup deviations, officials impact, per-player confidence) that the models can read without re-deriving them every prediction. (3) **The agentic build system itself** — the Opus-plans / Sonnet-executes / Sonnet-mirrors-to-RunPod / auditor-validates loop is the reason a one-engineer team can ship at this rate. Most quant shops cannot afford to debug 30 bugs in one night.
