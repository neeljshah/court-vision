...
const fixture = { sports: { soccer_intl: { n_rows: 9003, model_prob: { n: 9003, brier: 0.227887, reliability: 0.092837, resolution: 0.038185, uncertainty: 0.238877, reconstructed_brier: 0.293529 }, market_prob: { n: 9003, brier: 0.142726, reliability: 0.053423, resolution: 0.082761, uncertainty: 0.238877, reconstructed_brier: 0.209538 } } } };

describe("score decomposition data", () => {
  it("computes the signed reconstruction remainder", () => {
    expect(buildScoreDecomposition(fixture)[0].rows[0].remainder).toBe(0.065642);
  });
  ...
  it("retains the sport and source denominators", () => {
    const decomposition = buildScoreDecomposition(fixture)[0];
    expect(decomposition.nRows).toBe(9003);
    expect(decomposition.rows.map(row => row.n)).toEqual([9003, 9003]);
  });
