...
describe("Scout integrity metadata", () => {
  ...
  it("carries every inspector's complete registered source list", () => {
    ...
    expect(scoutIntegrity(calibration!.a).notices).toHaveLength(1);
  });
  ...
  it("uses exact explainer and paper citations and leaves safe records clear", () => {
    ...
    expect(scoutIntegrity(reliability.a).notices).toHaveLength(1);
    expect(paper.a.source_module_ids).toEqual(["murphy_decomposition", "why_attribution"]);
    expect(scoutIntegrity(paper.a).notices.length).toBeGreaterThan(0);
    expect(scoutIntegrity(paper.a).notices.every(notice => notice.status === "regenerated")).toBe(true);
