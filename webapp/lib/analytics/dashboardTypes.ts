...
export type DossierHistogramBin = { categoriesFilled: number; dossiers: number; share: number };
export type DossierCoverage = {
  n: number; median: number; categoriesTotal: number;
  histogram: DossierHistogramBin[]; medianCategories: number; medianCategoriesShare: number;
  publishedCompletenessScore: number; rates: { name: string; value: number }[];
};
