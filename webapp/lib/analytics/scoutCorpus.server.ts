...
function sources(): ScoutSources {
  ...
  const explainers = readRequired<{ essays: ScoutSources["explainers"] }>("explainers/explainers.json").essays.map(essay => ({ ...essay, sourceModuleIds: moduleIdsForCitations(essay.cited) }));
  return { curated: readRequired<ScoutSources["curated"]>("ask/corpus.json"), manifests: Object.fromEntries(packs.map(id => [id, readRequired<ScoutSources["manifests"][string]>(`showcase/${id}.json`)])), siteManifest, inspectorArtifacts, explainers, papers };
}
