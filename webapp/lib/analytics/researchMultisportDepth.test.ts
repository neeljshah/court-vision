...
describe("getMultisportDepthResearch", () => {
  ...
  it("keeps support, shrinkage movement, and rejected surface deltas explicit", () => {
    ...
    expect(soccer.rows.reduce((sum, row) => sum + (row.values.support_share || 0), 0)).toBeCloseTo(1, 10);
    ...
    shrinkage.rows.forEach(row => expect(row.values.regression).toBeCloseTo(row.values.shrunk_rate! - row.values.raw_rate!, 10));
    expect(shrinkage.rows.find(row => row.label === "Drew Millas")?.values.regression).toBeCloseTo(0.0408, 10);
