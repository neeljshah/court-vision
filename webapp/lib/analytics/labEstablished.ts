...
export function establishedDatasets(): LabDataset[] {
  ...
  const lineup = snapshot<{ top: SourceRow[]; bottom: SourceRow[] }>("lineup_synergy");
  const lineupFields = [f("synergy_residual", "Synergy residual / 48"), f("net_per48", "Observed net / 48"), f("expected_net_per48", "Expected net / 48"), f("min", "Minutes"), f("n_games", "Games", "number", 0)];
  const lineupRows = (rows: SourceRow[]) => rows.map(r => ({ ...r, name: (r.members as string[]).map(n => n.split(" ").slice(-1)[0]).join(" / "), note: (r.members as string[]).join(", ") }));
  output.push({ id: "lineup-synergy", title: "Five-player lineup synergy", sport: "nba", category: "Player & team", source: "lineup_synergy", description: "Compare a lineup's observed net rating with its player-based expected rating.", scope: "Published top and bottom lineups from 102 qualifying units, 2024-25.", caveat: "Roughly 100-minute floor; one season and unadjusted opponents. Residuals describe association and include roster and selection effects.", status: "Descriptive", fields: lineupFields, rows: [...labRows(lineupRows(lineup.top), lineupFields, "name", "Top residuals"), ...labRows(lineupRows(lineup.bottom), lineupFields, "name", "Bottom residuals")] });
