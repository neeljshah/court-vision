...
export type Retraction = {
  ...
  defect: string;
  /** When the withdrawal happened, where the evidence packet records that date; null otherwise. */
  eventDate: string | null;
  /** When the withdrawal record itself was published. Not the same as the withdrawal happening. */
...
export const RETRACTIONS: readonly Retraction[] = [
  ...
  },
  {
    ...
    eventDate: null,
    editorialDate: "2026-07-23",
    replacement: "Replacement measurement: leak-free walk-forward end-of-third-quarter Brier score of approximately 0.141 (unitless), stated as an approximation in JOB_EVIDENCE_PACKET.md and published there on 2026-07-23.",
...
export function validateRetractionCitations(retractions: readonly Retraction[]): void {
  for (const retraction of retractions) {
    ...
    // A citation may predate the editorial date, but not the event it is cited for.
    if (citation.date < (retraction.eventDate || retraction.editorialDate) && !citation.recordsWithdrawal) {
