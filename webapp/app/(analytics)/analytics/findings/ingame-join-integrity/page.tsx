...
export default function IngameJoinIntegrityPage() {
  ...
  const finding = buildIngameJoinIntegrityFinding(receipt, regeneration, timing);
  return <div className="wrap" style={{ paddingTop: 48, paddingBottom: 64 }}>
    ...
    <section style={section} aria-labelledby="measured-counts">
      ...
      <div role="region" aria-label="Corpus integrity counts" data-scroll-region tabIndex={0} style={{ overflowX: "auto", marginTop: 12 }}>
    ...
    <section style={section} aria-labelledby="tick-agreement">
      ...
      <div role="region" aria-label="Tick-level label agreement" data-scroll-region tabIndex={0} style={{ overflowX: "auto", marginTop: 12 }}>
    ...
    <section style={section} aria-labelledby="regeneration">
      ...
      <div role="region" aria-label="Artifact headline before and after regeneration" data-scroll-region tabIndex={0} style={{ overflowX: "auto", marginTop: 12 }}>
    ...
    <section style={section} aria-labelledby="timing-regeneration">
      ...
      <div role="region" aria-label="Timing artifact headline before and after regeneration" data-scroll-region tabIndex={0} style={{ overflowX: "auto", marginTop: 12 }}>
        <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 960 }}>
          <thead><tr><th style={tableHead}>Artifact</th><th style={tableHead}>Population</th><th style={tableHead}>n before</th><th style={tableHead}>n after</th><th style={tableHead}>Headline before (revision 1, withdrawn)</th><th style={tableHead}>Headline after (revision 2)</th></tr></thead>
          <tbody>{finding.timingRegeneration.rows.map(row => <tr key={row.artifact}><th scope="row" className="mono" style={{ ...tableCell, color: "var(--ink)", fontWeight: 600 }}>{row.artifact}</th><td style={tableCell}>{row.population}</td><td className="mono" style={tableCell}>{row.nBefore}</td><td className="mono" style={tableCell}>{row.nAfter}</td><td style={tableCell}>{row.headlineBefore}</td><td style={tableCell}>{row.headlineAfter}</td></tr>)}</tbody>
        </table>
      </div>
    ...
    <section style={section} aria-labelledby="exposed-artifacts"><h2 id="exposed-artifacts" className="serif" style={{ fontWeight: 500, fontSize: 24 }}>Affected calibration artifacts</h2><p style={{ ...lede, fontSize: 15 }}>The receipt records these artifacts against outcome labels. Their MLB and international soccer rows were rebuilt from the segment-clean corpus and now read revision 2.</p><ul className="mono" style={{ columns: "repeat(auto-fit,minmax(230px,1fr))", marginTop: 14, paddingLeft: 20, color: "var(--ink-2)", fontSize: 13 }}>{finding.exposedArtifacts.map(id => <li key={id} style={{ marginBottom: 4 }}>{id}</li>)}</ul><p style={{ ...lede, fontSize: 14 }}>Timing artifacts regenerated on {finding.timingRegeneration.measuredOn}: {finding.timingRegeneratedArtifacts.join(", ")}. Timing artifacts still under review: {finding.timingArtifacts.length ? finding.timingArtifacts.join(", ") : "none"}. {finding.timingNote}</p></section>
    <section style={section} aria-labelledby="status"><h2 id="status" className="serif" style={{ fontWeight: 500, fontSize: 24 }}>Artifact status registry</h2><p style={{ ...lede, fontSize: 15 }}>Only non-clear artifact and sport rows are listed. NBA and tennis rows remain clear.</p><div role="region" aria-label="Artifact status registry" data-scroll-region tabIndex={0} style={{ overflowX: "auto", marginTop: 12 }}><table style={{ borderCollapse: "collapse", width: "100%", minWidth: 620 }}><thead><tr><th style={tableHead}>Artifact</th><th style={tableHead}>Sport</th><th style={tableHead}>Status</th></tr></thead><tbody>{finding.statusRows.map(row => <tr key={`${row.artifact}-${row.sport}`}><th scope="row" className="mono" style={{ ...tableCell, color: "var(--ink)", fontWeight: 600 }}>{row.artifact}</th><td style={tableCell}>{row.sport}</td><td className="mono" style={tableCell}>{row.state}</td></tr>)}</tbody></table></div>{finding.derivedUnderReview.map(entry => <p key={entry.artifact} style={note}><span className="mono">{entry.artifact}</span> is under review because it was composed from an input that has since been rebuilt. {entry.note} Stale input: {entry.stale_inputs}.</p>)}</section>
    <section style={section} aria-labelledby="clear-status"><h2 id="clear-status" className="serif" style={{ fontWeight: 500, fontSize: 24 }}>What clears the status</h2><ul style={{ ...lede, fontSize: 15, paddingLeft: 22 }}><li>Done: segment identity checks keep every tick within its real game, and the checker reports no label disagreement and no multi-game file on the segmented corpus.</li><li>Done: the affected artifacts were regenerated from that checked corpus and published as revision 2.</li><li>Done: the six timing artifacts were rebuilt from the same corpus on {finding.timingRegeneration.measuredOn} and published as revision 2, and comeback_atlas was cleared on the separate evidence that its corpus is the NBA reliability map, which this segmentation cannot touch. No artifact remains under review.</li><li>Outstanding: the research papers still quote revision 1 numbers and must be re-audited against the regenerated artifacts.</li></ul></section>
  </div>;
}
