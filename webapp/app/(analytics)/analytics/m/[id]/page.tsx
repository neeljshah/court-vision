...
import { getTimingModuleEvidence } from "@/lib/analytics/timingModuleEvidence";
...
export function generateMetadata({ params }: { params: { id: string } }): Metadata {
  ...
  const timing = getTimingModuleEvidence(mod.id, readJson<Out>(join(data, "showcase", `${mod.id}.json`)) || {});
  const description = timing?.summary || (mod.one_line?.trim() && !/^descriptive_only$/i.test(mod.one_line) ? mod.one_line : "Published analytics module");
  return { title: timing?.title || insight?.title || mod.title, description };
}
