...
export default function ResearchDetail({ analysis: a, related }: { analysis: ResearchAnalysis; related: { id: string; title: string }[] }) {
  ...
  const selected = filtered.find(r => r.id === state.row);
  ...
  return <div className="cv-workspace research-page"><div className="cv-workspace-inner">
    ...
    <div className="research-layout"><section className="cv-panel research-measurements" aria-label="Interactive analysis">
      <div className="research-panel-title"><div><p className="cv-eyebrow">Explore the measurements</p><h2>Change the lens.</h2></div><button className="lab-export" disabled={!rows.length} onClick={() => exportLabCSV(a, rows)}><Download size={14} /> Export CSV</button></div>
      ...
      <div className="research-chart-toolbar"><div className="cv-segment">{[["rank", "Ranked bars"], ["scatter", "Scatter plot"], ["table", "Data table"]].map(([id, label]) => <button key={id} aria-pressed={view === id} onClick={() => change({ view: id })}>{label}</button>)}</div><label className="research-search"><Search size={14} /><span className="sr-only">Search analysis rows</span><input value={query} placeholder={a.id.includes("matchup") ? "Find a team or pairing" : "Find a row"} onChange={e => change({ query: e.target.value, row: "" })} /></label></div>
      ...
      <div className="research-view-actions"><button className="lab-export" onClick={copyView}><Link2 size={14} /> Copy view link</button><button className="lab-export" onClick={reset}><RotateCcw size={14} /> Reset view</button><span aria-live="polite" aria-label="Link sharing status">{copyStatus}</span></div>
      ...
      {!filtered.length ? <div className="cv-empty">No published rows match this search and population.</div> : view === "table" ? <LabTable key={`${group}-${query}`} dataset={a} rows={rows} onSelect={inspect} /> : view === "scatter" ? <ScatterPlot rows={filtered} x={field} y={y} onSelect={inspect} /> : ranked.length ? <RankedPlot rows={ranked} field={field} onSelect={inspect} /> : <div className="cv-empty">This measurement is unavailable for the selected population. The data table preserves missing values.</div>}
