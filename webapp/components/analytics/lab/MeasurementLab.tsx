...
export default function MeasurementLab({ data }: { data: LabData }) {
  ...
  const activeCohort = comparison.cohorts.find(cohort => cohort.key === selectedCohortKey) || comparison.cohorts[0];
  ...
  const suppressPooledSummaries = comparison.compatibility !== "compatible" && showAllCohorts;
  ...
  const choose = (value: string) => {
    ...
    update({ id: next.id, fieldKey: next.fields[0].key, otherKey: next.fields[1]?.key || next.fields[0].key, query: "", group: "all", selectedId: null, cohort: null, allCohorts: false });
  };
  const chooseSport = (nextSport: Sport) => {
    ...
    update({ sport: nextSport, id: next.id, fieldKey: next.fields[0].key, otherKey: next.fields[1]?.key || next.fields[0].key, group: "all", query: "", selectedId: null, cohort: null, allCohorts: false });
  };
  useEffect(() => {
    const restore = () => {
      setRestored(false);
  ...
  return <div className="cv-workspace"><div className="cv-workspace-inner lab-page"><header className="cv-masthead"><div><p className="cv-eyebrow">CourtVision / Measurement lab</p><h1>Explore the published measurements.</h1><p>Compare measurements, inspect rows, and export your selection.</p></div><a className="cv-primary" href={`${base}/analytics/compare/`}>Compare profiles <ArrowUpRight size={16} /></a></header>
    <div className="lab-shell"><aside className="lab-sidebar"><label className="lab-field-label">Explore a sport<select value={sport} aria-label="Filter lab by sport" onChange={e => chooseSport(e.target.value as Sport)}>{SPORTS.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}</select></label>
      ...
      {comparison.compatibility !== "compatible" && <section className="lab-cohort-notice" aria-label="Cohort comparison notice"><p>{comparison.reason} {showAllCohorts ? "All cohorts are shown without a pooled ranking or median." : `Showing ${activeCohort?.label || "one cohort"} by default.`}</p><ul>{comparison.cohorts.map(cohort => <li key={cohort.key}><b>{cohort.label}</b><span>{rowLabel(cohort.rowCount)}</span></li>)}</ul></section>}
