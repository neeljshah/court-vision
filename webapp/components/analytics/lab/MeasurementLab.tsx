...
import { labComparisonPolicy, type LabCohort } from "@/lib/analytics/labComparisonPolicy";
...
export default function MeasurementLab({ data }: { data: LabData }) {
  ...
  const rowsForCohort = (cohort: LabCohort) => sportRows.filter(row => Object.entries(cohort.definition).every(([key, value]) => row.definition?.[key as keyof NonNullable<LabRow["definition"]>] === value));
  const cohortForGroup = comparison.cohorts.find(cohort => rowsForCohort(cohort).some(row => row.group === group));
  const activeCohort = cohortForGroup || comparison.cohorts[0];
  const cohortRows = comparison.compatible || showAllCohorts ? sportRows : activeCohort ? rowsForCohort(activeCohort) : sportRows;
  const groups = Array.from(new Set(sportRows.map(r => r.group)));
  const groupRows = comparison.compatible ? sportRows.filter(r => group === "all" || r.group === group) : cohortRows;
  ...
  const suppressPooledSummaries = !comparison.compatible && showAllCohorts;
  ...
  return <div className="cv-workspace"><div className="cv-workspace-inner lab-page"><header className="cv-masthead"><div><p className="cv-eyebrow">CourtVision / Measurement lab</p><h1>Explore the published measurements.</h1><p>Compare measurements, inspect rows, and export your selection.</p></div><a className="cv-primary" href={`${base}/analytics/compare/`}>Compare profiles <ArrowUpRight size={16} /></a></header>
    <div className="lab-shell"><aside className="lab-sidebar"><label className="lab-field-label">Explore a sport<select value={sport} aria-label="Filter lab by sport" onChange={e => chooseSport(e.target.value as Sport)}>{SPORTS.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}</select></label>
      ...
      <div className="lab-controls"><label className="lab-field-label lab-primary-field">Primary measurement<select aria-label="Primary measurement" value={field.key} onChange={e => update({ fieldKey: e.target.value, selectedId: null })}>{dataset.fields.map(f => <option key={f.key} value={f.key}>{f.label}</option>)}</select></label><label className="lab-field-label">Published group<select aria-label="Published group" value={comparison.compatible ? group : showAllCohorts ? "all" : activeCohort?.key} onChange={e => { if (comparison.compatible) update({ group: e.target.value, selectedId: null }); else if (e.target.value === "all") { setShowAllCohorts(true); update({ group: "all", selectedId: null }); } else { const cohort = comparison.cohorts.find(item => item.key === e.target.value); setShowAllCohorts(false); update({ group: rowsForCohort(cohort || comparison.cohorts[0])[0]?.group || "all", selectedId: null }); } }}>
        {comparison.compatible ? <><option value="all">All published groups</option>{groups.map(g => <option key={g}>{g}</option>)}</> : <><option value="all">Show all cohorts (not comparable)</option>{comparison.cohorts.map(cohort => <option key={cohort.key} value={cohort.key}>{cohort.label}</option>)}</>}</select></label><label className="lab-field-label">Rank order<select aria-label="Rank order" value={ascending ? "asc" : "desc"} onChange={e => update({ ascending: e.target.value === "asc" })}><option value="desc">Highest first</option><option value="asc">Lowest first</option></select></label></div>
      {!comparison.compatible && <section className="lab-cohort-notice" aria-label="Cohort comparison notice"><p>{comparison.reason} {showAllCohorts ? "All cohorts are shown without a pooled ranking or median." : `Showing ${activeCohort?.label || "one cohort"} by default.`}</p><ul>{comparison.cohorts.map(cohort => <li key={cohort.key}><b>{cohort.label}</b><span>{rowLabel(cohort.rowCount)}</span></li>)}</ul></section>}
