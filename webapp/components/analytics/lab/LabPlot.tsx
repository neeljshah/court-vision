...
export function ScatterPlot({ rows, x, y, onSelect }: { rows: LabRow[]; x: LabField; y: LabField; onSelect: (r: LabRow) => void }) {
  ...
  </svg><p className="cv-footnote">{paired.length} paired {paired.length === 1 ? "row" : "rows"}. Axes span the selected data; constant measurements receive symmetric axis padding. This plot does not estimate a causal relationship or a fitted prediction.</p></div>;
}
