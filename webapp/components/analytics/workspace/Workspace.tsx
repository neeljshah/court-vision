...
export default function Workspace({ data, calibrationExample, paperCount, novelCount, findingCount }: {
  data: DashboardData & { snapshot: DashboardSnapshot };
  ...
  return <div className="cv-workspace cv-overview"><div className="cv-workspace-inner">
    <WorkspaceIntro example={calibrationExample} paperCount={paperCount} novelCount={novelCount} findingCount={findingCount} snapshot={snapshot} />
    <div className="cv-controls"><div className="cv-sports" role="group" aria-label="Filter analytics by sport">{SPORTS.map(s => <button key={s.id} onClick={() => setSport(s.id)} aria-pressed={sport === s.id} disabled={view === "library"}>{s.label}</button>)}</div><span className="cv-snapshot"><span />Published snapshot <b>{snapshot.asOf || "published snapshot"}</b></span></div>
    ...
    <div className="cv-view" key={view}>
      ...
      {(view === "overview" || view === "quality") && <Quality data={data} sport={sport} snapshot={snapshot} />}
      {view === "quality" && <Benchmarks data={data} sport={sport} snapshot={snapshot} />}
      {view === "overview" && <><div className="cv-overview-bottom"><Panel title="Published research outcomes" eyebrow="Research records" source="mechanism_ledger_export"><div className="cv-legend"><span><i className="cv-dot cv-confirmed" />Confirmed group</span><span><i className="cv-dot cv-null" />Null / other</span><span><i className="cv-dot cv-not_testable" />Not testable</span></div><div className="cv-verdict-chart">{SPORTS.filter(s => s.id !== "all" && (sport === "all" || s.id === sport)).map(s => { const rs = data.mechanisms.filter(r => r.sport === s.id); return <div key={s.id}><div className="cv-chart-label"><strong>{s.label}</strong><span>{rs.length} records</span></div><div className="cv-stacked" role="img" aria-label={`${s.label}: ${["confirmed", "null", "not_testable"].map(b => `${rs.filter(r => r.bucket === b).length} ${b}`).join(", ")}`}>{["confirmed", "null", "not_testable"].map(b => { const n = rs.filter(r => r.bucket === b).length; return <span key={b} className={`cv-${b}`} style={{ width: `${n / rs.length * 100}%` }}>{n}</span>; })}</div></div>; })}</div><button className="cv-text-button" onClick={() => setView("research")}>Inspect every research record <ArrowRight size={15} /></button><p className="cv-footnote">Grouped as recorded in the ledger. Local confirmation is not proof of causality. Open records to distinguish provisional and null findings.</p></Panel><Panel title="Recently added analyses" eyebrow="Derived analyses" className="cv-discover">{data.recentAnalyses.map((analysis, index) => <a href={`${base}/analytics/research/${analysis.id}/`} key={analysis.id}><span>{String(index + 1).padStart(2, "0")}</span><div><h3>{analysis.title}</h3><p>{SPORTS.find(s => s.id === analysis.sport)?.label || "Cross-sport"} / {number(analysis.rows)} {analysis.rows === 1 ? "row" : "rows"}</p></div><ArrowUpRight size={19} /></a>)}</Panel><Panel title="Related analysis pages" eyebrow="Analysis pages" className="cv-discover"><a href={`${base}/analytics/novel/`}><span>01</span><div><h3>Experimental measurements</h3><p>{novelCount} experimental measurements for lineups, fatigue, forecasts, and dependence.</p></div><ArrowUpRight size={19} /></a><a href={`${base}/analytics/findings/`}><span>02</span><div><h3>Published findings</h3><p>Measured results with their limitations and supporting evidence.</p></div><ArrowUpRight size={19} /></a><a href={`${base}/analytics/compare/`}><span>03</span><div><h3>Player comparisons</h3><p>Statistical profiles and available percentile comparisons.</p></div><ArrowUpRight size={19} /></a><a href={`${base}/analytics/the-loop/`}><span>04</span><div><h3>Research checks</h3><p>Testing steps and their recorded outcomes.</p></div><ArrowUpRight size={19} /></a></Panel></div><Coverage data={data} sport={sport} /></>}
