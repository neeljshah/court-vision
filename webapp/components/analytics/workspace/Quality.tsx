...
export function Quality({ data, sport, snapshot }: { data: DashboardData; sport: Sport; snapshot: DashboardSnapshot }) {
  ...
  return <div className="cv-quality"><Panel title="Model vs. market" eyebrow="Probability quality" source="calibration_by_market_type">
    ...
    {markets.length === 0 && <Empty>No market-type scores published for this sport in this snapshot. <Link href="/analytics/browse/">Open the Library.</Link></Empty>}
    ...
    <p className="cv-muted">Same metric, separate sport cohorts. {snapshot.monthCount ? `Only the ${snapshot.monthCount} published ${snapshot.monthCount === 1 ? "month" : "months"} are shown.` : "Published snapshot."}</p>
    {history.length === 0 ? <Empty>No monthly quality series published for this sport. <Link href="/analytics/calibration/">Open calibration records.</Link></Empty> : <div className="cv-history-grid">{(["mlb", "soccer"] as const).filter(s => sport === "all" || sport === s).map(s => {
    ...
    <p className="cv-footnote">{snapshot.monthRange || "Published snapshot"}. Changes in cohort composition can change these scores. Connecting observations is a visual guide, not an interpolated estimate.</p>
  </Panel></div>;
}
