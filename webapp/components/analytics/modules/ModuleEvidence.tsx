...
export function ModuleEvidence({ evidence, moduleId }: { evidence: Evidence; moduleId?: string }) {
  ...
  return <section className="module-evidence" aria-label="Published evidence status">
    ...
    {evidence.availability === "partial" ? <div><p className="overline">Published coverage</p><p>{evidence.headline}</p>{evidence.coverage.length ? <div className="module-evidence-scroll" role="region" tabIndex={0} aria-label="Published coverage (scrollable table)" data-scroll-region><table><thead><tr><th>Population</th><th>Status</th><th>Games</th><th>Usable buckets</th><th>Published reason</th></tr></thead><tbody>{evidence.coverage.map(row => <tr key={row.population}><td>{row.population}</td><td>{row.status.replace(/_/g, " ")}</td><td>{row.nGamesTotal ?? "-"}</td><td>{row.nBucketsUsable ?? "-"}</td><td>{row.reason || "-"}</td></tr>)}</tbody></table></div> : null}</div> : null}
