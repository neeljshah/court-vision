...
[WAVE 22 CONSUMED 2026-07-04 ~15:10Z -- all 5 items done, ledger above. Queue below = wave 23.]
1. [OWN-CLOSES-2023-EXTEND] the 2023 PM pilot lands 0 rows in nba_close_corpus (tip-time reference cache starts 2024-11-15): source tip times from OUR OWN data (linescores.parquet / ESPN schedule ingest) for 2023-2024 -> extend the corpus back (+435 candidate games) and re-run the benchmark as a TWO-SPLIT verdict (2023-24 vs 2024-26) | scripts/platformkit/venue_history/ | corpus rows from 2023 + per-split honest verdicts.
2. [LIVE-ACTIVATION-VERIFY] once MLB slate is live (~17:00Z+): enrichment fields present in fresh grade rows, kalshi pacing counters nonzero, m13 fresh props_snapshot + SLA green, grade-writer clamp writing late-inning rows; npb/kbo FIRST GRADES land overnight (their game windows) | orchestrator light check-in | verifier PENDINGs -> PASS or triaged.
3. [LIVENESS-TEST-ISOLATION] ops liveness heartbeat() writes the real production heartbeat path when any test calls it without an injected path (observed 1970-01-01 stamp on m13's file during wave-22 concurrent lanes) -> add injectable-path guard + audit callers | find the liveness module under scripts/platformkit/ | hazard closed, per-file tests green.
4. [SOCCER-ENRICH-SPLIT] enrichment_rows_soccer.py at 345 LOC (Opus advisory, pre-existing overage) -> split cleanly | scripts/platformkit/ingame/ | both files <=300, tests green.
5. [FORWARD-GATES-WATCH] wall-clock accrual, no fleet: MLB tail fwd (floor ~20), soccer_intl xG forward (floor 8/half), wnba/npb/kbo tail stamps live -- escalate to a decision wave ONLY on a real headline_verdict or an allowlist-external regression | forward_evidence_scoreboard | gate decides -> decision wave.
