...
0b. [WAVE-27 ASK-ANYTHING ENGINE, launches when 26 lands -- USER EXPANSION 2026-07-04d: "not just shooting, every little aspect of sports... an LLM I can just ask any question using my system and claude running it, actually accurate analytics"] (a) generalize the claims engine across ALL 28 player-atlas + 13 team-atlas dimensions (playmaking, defense, rebounding, clutch, durability, schemes...) -- one metric-spec registry, same provenance/floors/validator contract; (b) ask-anything interface = a repo SKILL (Claude = the LLM layer): NL question -> metric spec -> computed + independently-validated answer with provenance + caveats; refuses when data cannot support (honest UNANSWERABLE); (c) LLM-into-predictions stays gated by the L4 planted-null gate from wave 26 -- narrative/scouting NEVER feeds predictions outside it | scripts/platformkit/intel_query/ + .claude/skills/ | ask "who leads X" for any atlas dimension and get a validated, provenance-carrying answer.
