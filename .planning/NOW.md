...
[wave 12 queue (historical-odds scout synthesis, 2026-07-04 late): 1. POLYMARKET INTRA-GAME BACKFILL (TOP -- adjudicates H1/H2 tails on 2023-2026 multi-season validation corpora): scripts/platformkit/venue_history/polymarket_intragame.py per synthesis spec (dailies-slug enumeration, explicit startTs/endTs fidelity=1, provenance-separate corpus, NEVER pooled with forward gates) + run the tail-band scan on it as an INDEPENDENT validation verdict. 2. KALSHI INTRA-GAME BACKFILL (2-3mo depth, dense 1-min candles + tape): venue_history/kalshi_intragame.py (cursor/max_close_ts pagination; min_close_ts ignored). 3. WTA ODDS INGEST (tennis-data.co.uk -- un-PENDs the WTA close comparison; stealth transport if TLS alert persists). 4. ingame-prop-channel root-cause (carryover). 5. espn_wp fuzzy resolver for 36 unmatched games + WNBA CDN egress retry (carryover).
HUMAN QUEUE ADDITION: OddsAPI purchase brief -- the ONE genuine paid gap is NBA multi-book Pinnacle-anchored pregame closes 2020-2026 (~55K credits = one $59 month on the 100K tier; in-play history NOT offered so venue routes cover the in-play program free). Your call; never auto-purchased.]
