...
1. [XG-APPLY, user-ratified] apply PROPOSED_soccer_xg_wiring.md: fresh Opus adversarial review + gate re-run (walk-forward, >=2 corpora) before commit | domains/soccer_intl/ + scripts/platformkit/ingame/ | soccer in-game re-verdict recorded (expect WORSE->MATCH movement; calibration only, no market claim).
2. [OWN-NBA-CLOSES] distill OUR OWN NBA pregame close corpus from PM intra-game 2023-2026 + Kalshi candles (last strictly-pregame tick per game = venue close proxy, provenance-labeled VALIDATION corpus, never pooled with forward gates) + benchmark the NBA pregame model against it | scripts/platformkit/venue_history/ + domains/basketball_nba/ | close-corpus parquet + honest benchmark verdict.
3. [HIST-SELF-IMPROVE] owned intra-game tick corpora as REFIT fuel: WNBA blend refit on >=2 corpora (queued since wake 2) + MLB in-game calibration refit on PM paths; walk-forward, provenance-separated, gated vs venue | domains/wnba/ + domains/mlb/ | gate verdicts (REJECT = success).
4. [ADV-CQR] Conformalized Quantile Regression (ADVANCED_TECHNIQUES T1) for NBA prop intervals + Mondrian per-regime coverage (T2), spec gates exactly (planted-null, proper-score unanimity, >=2 corpora, default-OFF) | domains/basketball_nba/prop_cqr.py | SHIP/REJECT verdict rows.
5. [LIVE-BREADTH + NBA-READINESS] (a) WNBA CDN live retry (egress recheck) + espn_wp reference -> wnba + Naver KBO relay re-probe (stealth, honest blocked-note ok); (b) NBA states corpus from linescores.parquet -> run the ported NBA states gate + espn_wp nba probe | scripts/platformkit/ingame/ + domains/basketball_nba/ | new enrichment fields or blocked-notes; states-gate verdict at real n.
HUMAN QUEUE (updated 2026-07-04 wave 22): restart DONE (user-attended, this wake); xG wiring RATIFIED-APPLY (item 1); OddsAPI DECLINED (build-own ratified). Still open: soccer-suppression memo; m32 weather_totals SHIP_REVIEW; states-gate CI adoption at bigger n; prop-guard KEEP ack; UI-off option; reconcile_survivors adopt design; register_autostart.ps1 -Register in an ELEVATED shell (standing).
