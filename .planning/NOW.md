...
1. [P3.4] Widen the m19 asof-reclaim enumerator with tennis (asof_hold.parquet, ATP-only -- needs a WTA companion asof_hold extractor before it can clear pregame_feature_gate's cross-corpus SHIP bar) and soccer (asof_features.parquet diff_sot/shots_for/against_asof as a direct logit feature, distinct from the poisson_xg candidate already gated) | scripts/platformkit/ceiling/ + domains/tennis|soccer | new scoreboard rows; REJECT expected, any SHIP_REVIEW surfaces to the human
...
5. MLB DEEP-INTELLIGENCE CONTINUATION (WAKE-31 opened these; verify-live-first): (a) HUMAN DECISION: flip CV_MLB_SP_ADJUST=1 in predict_service's MLB slate producer? (gate PASSED cross-era + 2026 OOS, verdict JSON on disk); (b) build the un-deferred asof_bullpen feature (SP identity now in probables.parquet keyed by game_pk -- follow asof_bullpen_DEFER.md's own un-defer recipe, gate vs Elo); (c) umpire strike-zone tendencies: officials now snapshot per game -- backfill officials history + per-ump K/BB aggregation, gate as totals feature; (d) per-pitch archetype/matchup layer from the now-5-season pitch_states corpora (2022-2026, cross-corpus replication mandatory); (e) extend the same context layer to soccer/tennis once MLB is solid
