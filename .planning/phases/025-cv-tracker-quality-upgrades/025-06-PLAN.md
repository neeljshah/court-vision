...
IMPORTANT: The shape and dtype checks are collapsed INTO test_synthetic_court_returns_matrix.
Do NOT use pytest.skip() when detection returns None on a synthetic image — the synthetic
image SHOULD have detectable court lines, so a None return means the detector is broken
and the test must FAIL with a clear error message. The separate test_returned_matrix_shape
and test_returned_matrix_dtype_float64 functions from any prior draft are NOT included.
...
Write the file exactly as shown above. No modifications to the test logic.
  ...
  <done>
    ...
    - test_synthetic_court_returns_matrix passes — asserts non-None AND shape (3,3) AND dtype float64
    - No pytest.skip() calls in detection cases — None return on synthetic image is a FAIL
...
<success_criteria>
- tests/test_court_detector.py exists with 7 test functions
- Import test passes
- None-return tests pass (empty list, blank frames, uniform grey)
- test_synthetic_court_returns_matrix passes and asserts non-None, shape (3,3), dtype float64 in one block
- No pytest.skip() in detection cases — synthetic image failure is a real test failure
