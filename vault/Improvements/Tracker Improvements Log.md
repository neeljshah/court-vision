...
> [[MOC-CV]] | [[Sessions/Timeline]] | [[Tracking/Open Issues]] | [[Sessions/Game Log]]

Chronological log of CV pipeline fixes. Related notes:
- [[Improvements/ingest_hardening_complete]] — Ingest system hardening (Session 41)
- [[Improvements/ingest_preflight]] — Ingest preflight checks
- [[Improvements/API_Models_Readiness_2026-03-26]] — API + models readiness audit
...
### 2026-05-21 09:31 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 09:32 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 09:33 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 13:26 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 13:31 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 13:31 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 15:52 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 15:53 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 16:07 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 16:12 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 21:36 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 21:41 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 21:47 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 21:51 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 22:03 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 22:14 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 22:24 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-21 22:34 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-22 20:10 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-22 20:17 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-22 20:20 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-22 20:22 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-22 20:23 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain
...
### 2026-05-22 21:50 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-22 21:54 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

### 2026-05-22 22:11 -- Auto-Retrain
Milestone 20 games reached: triggering tier3 retrain

---

## Loop 5 Cycle 89-90 batch (in-game frontier kickoff) — 2026-05-24

### SHIPPED infra
- **89a** — `scripts/predict_in_game.py` snapshot schema unified to `src/data/live` canonical (commit b4198884)
- **89b** — `src/prediction/live_factors.py` `foul_trouble` collapsed 3 tables -> 1 conservative-bias table
- **89e** — `scripts/probe_inplay_vs_pregame.py` Q1/Q2/Q3 retro MAE framework (commit db675596)
- **89f** — research markdown `scripts/_results/in_game_gaps_v1.md` (4 tiers, 11 hypotheses, recommended ordering)
- **90e** — `scripts/fetch_player_positions.py` + `data/cache/playerinfo/` commonplayerinfo cache + positions parquet (commit 21000d93; ~50 pids cached so far)
- **90f** — rolling quantile calibration: `scripts/quantile_calibration_rolling.py`, `compare_to_lines.py --rolling-cal` flag, drift report at `scripts/_results/quantile_rolling_drift_v1.md` (commit cea28b2d)

### REJECTED probes
- **89c** per-position factors — REJECT: `position` MISSING from pergame rows; cheapest unlock is cycle 90e commonplayerinfo backfill
- **89d** per-quarter pace decay — REJECT: no per-quarter player splits on disk; Strategy B synthetic is self-consistent with uniform-decay assumption and cannot detect true coefficient
- **90a** T1-A garbage-time haircut — REJECT: no 2025-26 spread/SRS in `season_games.parquet`; best PTS effect -0.0020 (gate 0.005), REB regresses +0.0017, WF non-monotone (fold4 +0.0034)
- **90b** T1-C b2b × age33+ — REJECT: `is_b2b=0` for entire 2025-26 holdout (rest_travel.parquet ends 2025-04-13); on Q4 fallback window 276 affected rows PTS+REB worsen, AST barely moves
- **90c** T1-B foul-rate MIN shrink — REJECT (no result file shipped): PF column absent from gamelogs; BLK-as-foul-proxy too weak to drive a calibrated quintile cut
- **90d** T1-E REB OREB-context (`team_oreb_pct_l5 * opp_dreb_pct_l5`) — REJECT: single-split +0.0013, WF 1/4 folds positive (fold1 -0.0011, folds 2-4 +0.0008..+0.0024)

### Lessons learned (durable, quotable)
- **"Gamelog-only validation has SELECTION BIAS for sit-rate effects — players who DNP are silent in the data."** T1-C structural failure: the 80%-sit-on-b2b veteran prior is filtered OUT of `gamelog_*.json`; the 276 rows we tested are the 20% who DID suit up (healthy, in-form) so shrinking them moves predictions the wrong way. Any sit-rate hypothesis needs a DNP-aware projection set.
- **"Data gaps now bottleneck the in-game frontier more than algorithm gaps — 4 of 6 batch-2 probes blocked by missing data."** Positions, per-quarter splits, 2025-26 spreads, and 2025-26 rest_travel each killed a probe independently. The 89f T1 ordering was honest but the dataset isn't ready.
- **"Pre-game spread is NOT in `season_games` — depends on external scoreboard scrape."** SRS proxy `(home_srs - away_srs + 2.5)` covers through 2025-04-13 only; 2025-26 holdout has zero spread coverage. Schedule a closing-line fetcher as ops infra.
- **"Per-position stratification depends on commonplayerinfo unlock; only ~50 pids in cache so far."** 90e laid the infra; needs the full ~800-pid backfill before per-position probes are testable.
- **"Conditioning a saturated probe more narrowly doesn't rescue it if the conditioning column is itself dead in the holdout."** Both 90a (spread) and 90b (is_b2b) failed this way. Validate the conditioning column has signal on the holdout BEFORE running the adjustment sweep.

### Data unlock queue
1. Full ~800-pid `commonplayerinfo` fetch — extend cycle 90e cache (unblocks 89c)
2. Per-quarter boxscore parquet via `boxscoretraditionalv3` periods 1..4 — `scripts/fetch_per_quarter_boxscores.py` (cycle 91a)
3. PF in gamelogs (or PF/36 season aggregate) — unblocks T1-B (cycle 91b)
4. 2025-26 pre-game spreads — scoreboard / closing-line scrape (cycle 91c)
5. `rest_travel.parquet` rebuild through 2026-04-12 — `is_b2b` is dead for every 2025-26 inference today (cycle 91d)

### Open in-game frontier (retry queue, once data lands)
- T1-A garbage-time haircut retry with real 2025-26 spreads (after 91c)
- T1-B foul-rate MIN shrink retry with real PF (after 91b)
- T1-D Q1 pace residual as full-game multiplier (after 91a per-quarter parquet)
- T1-C b2b veteran retry — research DNP-aware projection set first; current gamelog-only framing is structurally untestable
- Per-position MAE strata + position-conditional REB/BLK adjustments (after 90e backfill completes)
