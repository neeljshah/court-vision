"""tests/platform/test_recalibration_leak.py

Leak battery for scripts/platformkit/recalibration.py::walk_forward_recalibrate.
...
Tests:
  ...
  (2) DETERMINISM — two identical calls produce bit-identical output.
  ...
  (5) NaN/inf INPUTS — don't crash; output stays finite/bounded.
...
from scripts.platformkit.recalibration import walk_forward_recalibrate  # noqa: E402

# ---------------------------------------------------------------------------
# Shared fixture: a synthetic sequence long enough to clear warmup (50)
# ---------------------------------------------------------------------------

_RNG = np.random.default_rng(42)
_N = 150
_MIN_H = 50  # matches the default; we pass it explicitly for clarity
...
def test_outcome_independence_permute():
    ...
    k = 80   # outcomes from index 80 onward are the "future"
    j = 60   # we inspect calibrated through index j (well inside warmup-cleared range)

    ref = walk_forward_recalibrate(_RAW, _OUT, min_history=_MIN_H)
    ...
    perturbed = walk_forward_recalibrate(_RAW, out_permuted, min_history=_MIN_H)
...
def test_outcome_independence_flip():
    """Flipping all outcomes[k:] must not affect calibrated[:j+1] for j < k."""
    k = 90
    j = 70
    ...
    flipped = walk_forward_recalibrate(_RAW, out_flipped, min_history=_MIN_H)
...
def test_outcome_independence_warmup_zone():
    ...
    cal_zero = walk_forward_recalibrate(_RAW, out_all_zero, min_history=_MIN_H)
    cal_one = walk_forward_recalibrate(_RAW, out_all_one, min_history=_MIN_H)

    # In warmup zone calibrated[i] == raw[i] always — outcomes are irrelevant.
...
def test_determinism():
    ...
    a = walk_forward_recalibrate(_RAW, _OUT, min_history=_MIN_H)
    b = walk_forward_recalibrate(_RAW, _OUT, min_history=_MIN_H)
    np.testing.assert_array_equal(
        a, b, err_msg="walk_forward_recalibrate is non-deterministic across two calls."
    )
...
def test_truncation_invariance(k: int):
    ...
    full = walk_forward_recalibrate(_RAW, _OUT, min_history=_MIN_H)
    prefix = walk_forward_recalibrate(_RAW[:k], _OUT[:k], min_history=_MIN_H)
...
def test_output_bounds():
    ...
    cal = walk_forward_recalibrate(_RAW, _OUT, min_history=_MIN_H)
...
def test_output_bounds_extreme_probs():
    ...
    cal = walk_forward_recalibrate(raw_extreme, out, min_history=_MIN_H)
    assert np.all(cal >= 0.0) and np.all(cal <= 1.0), (
