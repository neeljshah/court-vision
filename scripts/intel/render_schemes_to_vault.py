...
--------------------------
Render team scheme/identity atlas parquets into human-readable Markdown
notes under vault/Intelligence/Schemes/.

Writes:
  - vault/Intelligence/Schemes/<TRI>_schemes.md  (one per team, 30 files)
  - vault/Intelligence/Schemes/_Scheme_Matrix.md (30-team overview)

Safe to re-run — only overwrites files in vault/Intelligence/Schemes/
that match the *_schemes.md / _Scheme_Matrix.md pattern.
"""
...
DEFAULT_OUT = ROOT / "vault" / "Intelligence" / "Schemes"
...
def _write_team_note(tri: str, dfs: dict[str, pd.DataFrame], out_dir: Path) -> None:
    lines: list[str] = []
    lines.append(f"# {tri} — Scheme & Identity Atlas")
    lines.append("")
    lines.append(f"*Auto-generated by render_schemes_to_vault.py — do not edit manually*")
    lines.append("")
    ...
    out_path = out_dir / f"{tri}_schemes.md"
...
def _write_matrix(teams: list[str], dfs: dict[str, pd.DataFrame], out_dir: Path) -> None:
    col_names = [
        "Defensive",
        "Offensive",
        "Rebound",
    ...
    lines.append("")
    lines.append("Click any team link to see the full scheme scouting note.")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Index — Individual Team Notes")
    lines.append("")
    for tri in sorted(teams):
        lines.append(f"- [[{tri}_schemes]]")

    out_path = out_dir / "_Scheme_Matrix.md"
    out_path.write_text("\n".join(lines), encoding="utf-8")
...
def render_all(out_dir: Path | None = None) -> None:
    """Load all scheme parquets and render 30 team notes + matrix to out_dir."""
    if out_dir is None:
        out_dir = DEFAULT_OUT
    ...
    print(f"\nWriting team notes to {out_dir} …")
    written = 0
    for tri in sorted(teams):
        _write_team_note(tri, dfs, out_dir)
        written += 1

    print(f"  Wrote {written} team notes.")
    _write_matrix(sorted(teams), dfs, out_dir)
    print("Done.")
