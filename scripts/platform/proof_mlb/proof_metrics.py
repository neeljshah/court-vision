"""scripts.platform.proof_mlb.proof_metrics — Pure metric functions for the MLB moneyline proof.
...
def brier(probs: np.ndarray, outcomes: np.ndarray) -> float:
    ...
    probs:
        ...
        For MLB moneyline: P(home team wins = 1).
    outcomes:
...
def ece(probs: np.ndarray, outcomes: np.ndarray, bins: int = 10) -> float:
    ...
    Parameters
    ----------
    probs:    predicted P(home team wins).
    outcomes: binary labels (1 = home win, 0 = away win).
...
def isotonic_calibrate(
    ...
    Parameters
    ----------
    train_p:  raw moneyline home-win probabilities for the training season.
    train_y:  binary outcomes (1 = home win) for the training season.
    eval_p:   raw moneyline home-win probabilities for the held-out evaluation season.
...
def _devig2(price_a: float, price_b: float) -> Tuple[float, float]:
    ...
    Side A = home team; side B = away team (moneyline).
...
def clv_sign_invariants(
    ...
    Parameters
    ----------
    open_a, open_b:   decimal odds for side A (home) / B (away) at open.
    close_a, close_b: decimal odds for side A (home) / B (away) at close.
    ...
    dict with keys:
        ...
        mean_clv_a  (float): mean CLV betting side A (home).
        mean_clv_b  (float): mean CLV betting side B (away).
