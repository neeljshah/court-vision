"""scripts.platform.proof_soccer.proof_metrics — Pure metric functions for the soccer O/U-2.5 proof.
...
Side convention: side A = Over 2.5 goals / side B = Under 2.5 goals.
"""
...
def brier(probs: np.ndarray, outcomes: np.ndarray) -> float:
    ...
    probs:
        ...
        For soccer O/U-2.5: P(Over 2.5 goals = 1).
    outcomes:
...
def ece(probs: np.ndarray, outcomes: np.ndarray, bins: int = 10) -> float:
    ...
    Parameters
    ----------
    probs:    predicted P(Over 2.5 goals).
    outcomes: binary labels (1 = Over, 0 = Under).
...
def isotonic_calibrate(
    ...
    Parameters
    ----------
    train_p:  raw O/U-2.5 probabilities for the training season.
    train_y:  binary outcomes (1 = Over) for the training season.
    eval_p:   raw O/U-2.5 probabilities for the held-out evaluation season.

    Returns
    -------
    np.ndarray
        Calibrated probabilities for the eval season, shape (n_eval,).
    """
...
def _devig2(price_a: float, price_b: float) -> Tuple[float, float]:
    """Two-sided devig of decimal odds → fair implied probabilities.

    Side A = Over 2.5 goals; side B = Under 2.5 goals.
...
def clv_sign_invariants(
    ...
    Parameters
    ----------
    open_a, open_b:   decimal odds for side A (Over) / B (Under) at open.
    close_a, close_b: decimal odds for side A (Over) / B (Under) at close.
    ...
    dict with keys:
        ...
        mean_clv_a  (float): mean CLV betting side A (Over).
        mean_clv_b  (float): mean CLV betting side B (Under).
    ...
    open_prob_a = np.array([_devig2(oa, ob)[0] for oa, ob in zip(open_a, open_b)])
    close_prob_a = np.array([_devig2(ca, cb)[0] for ca, cb in zip(close_a, close_b)])
