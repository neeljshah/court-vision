"""backtest_pts_oos.py - iter-8 OOS backtest for PTS sqrt+Huber blend.

Loads the OOS PTS artifacts (XGB + LGB + MLP + scaler + optional isotonic
calibrator + NNLS weights) from data/models/oos_pre_playoffs/ and replays
the 2024 playoffs canonical CSV row-by-row, reusing _build_asof_row from
the iter-6 closing-line backtest for leak-free feature construction.

This mirrors scripts/backtest_qstat_oos.py but for the blend path (PTS is
not a q50 stat in production). Prediction faithfully reproduces
prop_pergame.predict_pergame for stat='pts':
    pred = NNLS(w_xgb*xgb + w_lgb*lgb + w_mlp*mlp) on raw-count scale,
            after per-base-learner sqrt-inverse (clip>=0; v**2)
    pred = isotonic_cal(pred)   # iff calibration_pergame_pts.joblib exists
    pred = garbage_time_haircut(pred, hs)   # cycle 96a
    pred = residual_correction(pred, row, "pts")   # passthrough per docstring

We bypass prop_pergame.predict_pergame() entirely to ensure we read the
OOS artifacts and OOS NNLS weights (not the production module-cached ones).

Reference (iter-4 in-sample, from closing_line_backtest_2024_playoffs.md):
    PTS  n_pred=853  n_bets=687  hit=55.17%  ROI=+5.32%
"""
...
# iter-4 in-sample reference (closing_line_backtest_2024_playoffs.md row for PTS).
IS_REF = {"hit": 0.5517, "roi": 5.32, "bets": 687, "n_pred": 853}
...
def _load_oos_artifacts():
    ...
    mlp_scaler_path = os.path.join(OOS_DIR, "props_pg_mlp_scaler_pts.pkl")
...
def _predict_blend(artifacts, feat_row: Dict[str, float]) -> Optional[float]:
    ...
    # Cycle 96a haircut (PTS is a volume stat — receives the multiplicative shrink).
...
def run() -> dict:
    ...
    # Load CSV — filter to PTS rows.
    all_rows = []
    with open(CSV_PATH, encoding="utf-8") as fh:
        for r in csv.DictReader(fh):
            if r.get("stat", "").lower() == STAT:
                all_rows.append(r)
    print(f"  CSV PTS rows: {len(all_rows)}")
...
def save_report(result: dict) -> str:
    ...
    L: List[str] = []
    L.append("# PTS OOS Backtest - iter-8\n")
    L.append("Leak-clean OOS backtest of the PTS sqrt+Huber blend vs 2024 playoff closing lines.\n")
    ...
    L += [
        ...
        f"## Verdict: **{verdict}**",
        "",
        "Threshold: |edge| > 0.5 PTS. Bet pricing: -110/-110. Verdict rule:",
