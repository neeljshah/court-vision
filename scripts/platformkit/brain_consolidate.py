...
_EXPLICIT: List[Tuple] = [
    ...
    ("Tennis", "Trends",    "2*.md", ("_",), "YearTrends",
     "Tennis style-trends year stubs", 3),
]
...
def _norm_key(k: str) -> str:
    k = k.lower().strip()
    k = re.sub(r"_(label|count|total)$", "", k)
    k = re.sub(r"^total_", "", k)
    k = re.sub(r"\s+in\s+corpus$", "", k)
    k = re.sub(r"^(typical\s+)?", "", k)
    return k.strip()
...
def _facts(text: str, stem: str) -> List[str]:
    ...
    if fm:
        for ln in fm.group(1).splitlines():
            ln = ln.strip()
            if not ln or ln.startswith("tags") or ln.startswith("-"):
                continue
            if "name:" in ln and stem.lower().replace(" ", "_") in ln.lower():
                continue
            raw.append(ln)
    for ln in _FM_RE.sub("", text).splitlines():
        ...
        # Skip table rows containing histogram glyphs
        if s.startswith("|") and _HIST.search(s): continue
    ...
    for fragment in raw:
        ...
        if not c: continue
        ci = c.find(":")
        raw_key = c[:ci].strip() if ci != -1 else c
...
def _repair_file(fp: Path, stems: Set[str], cons_rel: str, cons_name: str) -> int:
    ...
    for line in text.splitlines(keepends=True):
        ...
        for pat in pats.values():
            if pat.search(nl):
                nl = pat.sub(cons_link, nl); hit = True
        if hit:
            stripped = nl.strip().lstrip("- ").strip()
            only_cons = stripped == cons_link
            if only_cons and last_was_cons:
...
def _repair_sport(sport_dir: Path, stems: Set[str], cons_path: Path) -> int:
    ...
    for md in sport_dir.rglob("*.md"):
        ...
        try:
            rel = cons_path.relative_to(md.parent).as_posix().replace(".md", "")
        except ValueError:
            try:
                rel = cons_path.relative_to(sport_dir.parent).as_posix().replace(".md", "")
            except ValueError:
                rel = cons_path.stem
        cons_name = cons_path.stem.lstrip("_").replace("_", " ")
        total += _repair_file(md, stems, rel, cons_name)
    return total
...
def _detect_generic(sport_dir: Path, category: str) -> Optional[Tuple]:
    cat_dir = sport_dir / category
    if not cat_dir.is_dir(): return None
...
def consolidate(organized_root: Optional[Path] = None, write: bool = True,
    ...
    if injected_families is not None:
        for spec in injected_families:
            ...
            cat_dir = root / sport / cat
            cat_dir.mkdir(parents=True, exist_ok=True)
            members = [Path(m) for m in spec["members"]]
            sport_dir = root / sport
            if not members:
                by_sport.setdefault(sport, []).append(
                    {"family": spec["name"], "n_merged": 0, "n_removed": 0,
                     "n_links_repaired": 0,
                     "output": str(cat_dir / f"_{spec['name']}_Consolidated.md")})
                continue
...
def _main(argv: Optional[List[str]] = None) -> int:
    ...
    for sport, infos in rep["by_sport"].items():
        for info in infos:
            print(f"  [{sport}/{info['family']}] "
                  f"{info['n_merged']} stubs -> {info['output']}")
