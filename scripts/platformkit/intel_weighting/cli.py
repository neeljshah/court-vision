...
_SPORT_PREFIX = {"nba": "nba_", "mlb": "mlb_", "soccer": "soccer_", "tennis": "tennis_"}
# "soccer_" also prefix-matches the unrelated soccer_intl_* families (a
# different sport/corpus, data/domains/soccer_intl) -- exclude explicitly.
_SPORT_EXCLUDE_PREFIX = {"soccer": "soccer_intl_"}


def _families_for(sport: str) -> List[str]:
    prefix = _SPORT_PREFIX[sport]
    exclude = _SPORT_EXCLUDE_PREFIX.get(sport)
    return [f for f in discover_families()
            if f.startswith(prefix) and not (exclude and f.startswith(exclude))]
...
def _print_board(sport: str, results: List[GateResult]) -> None:
    ...
    print(f"{'family':<26}{'metric':<22}{'n':>5}  {'brier_base':>10} {'brier_cond':>10} "
          f"{'delta':>9}  verdict")
    print("-" * 96)
    order = {"MATTERS_PROVISIONAL": 0, "NULL": 1, "UNTESTABLE": 2}
