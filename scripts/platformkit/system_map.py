...
_INGAME = {
    "nba": "NBARepricer (Gaussian score-anchor). BACKTESTED on 1,313 games (per-quarter "
           "linescores) on the FULL PRIVATE corpus: COMBINED pregame-rating-prior + realized-score "
           "= Brier 0.159, beats pregame-Elo 0.209 AND score-only 0.172. "
           "VALIDATION_PENDING on committed fixture (fixture prints no-improvement = SYNTHETIC "
           "ANCHOR ARTIFACT; does NOT refute the real-corpus result). Usable: predictor.predict_live(). "
           "Per-quarter curve = null (quarters uniform). edge_claimed=False.",
...
def render_markdown(m: Dict) -> str:
    ...
    if isb:
        ...
        L += ["> **Canonical numbers + full why-text live in `_Ingame_Scoreboard.md`**, "
              ...
              "a hardcoded copy that could drift. Headline: MLB/Soccer/Tennis in-game "
              "conditioning WIN rows reproduce on committed fixtures. NBA in-game WIN is "
              "real-corpus-only (VALIDATION_PENDING on fixture = SYNTHETIC ANCHOR ARTIFACT); "
              "edge_claimed=False.", ""]
