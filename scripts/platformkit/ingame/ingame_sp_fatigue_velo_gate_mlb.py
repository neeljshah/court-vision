"""scripts.platformkit.ingame.ingame_sp_fatigue_velo_gate_mlb -- REDESIGN of the wave-29
MLB SP within-start fatigue in-game gate (lane 4, conductor rank 4). Wave-29's proxy
(pitch-count / velo_decline_vs_early terciles on the ESPN-only corpus, NO real pitcher
id) was NOT_TESTABLE: the planted null matched the real improvement exactly (dm_p=.020
real vs dm_p=.024 null, both "shipping") -- an added-flexibility artifact, not a fatigue
signal. This redesign swaps the fatigue proxy to WITHIN-START VELOCITY DECLINE computed
from REAL Statcast release_speed + a REAL per-pitcher identity (domains/mlb/
sp_velo_materialize.py), a strictly stronger identity than wave-29's (game_id,side)
staff-block substitute. If the null matches again here, the verdict is NOT_TESTABLE and
the hypothesis class CLOSES (per the pre-registration).

THE QUESTION (H1, redesigned proxy): does starting-pitcher within-start velocity decline
(velo_decline = trailing-last-15 minus first-15 release_speed, banded into
{fresh,declining,fatigued} terciles fit on TRAIN only) condition in-game win-prob Brier
BEYOND a pure (score_diff, frac_elapsed) BASE?

TWO MODELS over the reconstructed per-pitch as-of states (structure identical to the
PROVEN NBA/wave-29 clone pattern):
  BASE     = sigmoid((a + b*frac_elapsed) * state_diff)      # (margin,time) only
  +FATIGUE = sigmoid(logit(BASE) + beta[tercile])             # BASE + a per-tercile
             logit shift, beta fit on the TRAIN fold ONLY; tercile edges also TRAIN-only.

DISCIPLINE (per spec floors, identical to wave-29): >=3 expanding-window walk-forward
folds; min_cell=20 states per tercile before a beta is fit for it (else beta->0, trust
BASE); >=100 states per tercile pooled OOS; DM clustered by proxy_pitcher (here a REAL
pitcher id); PLANTED NULL = shuffle the tercile assignment across pitchers within the
same game-state cell (ingame_sp_fatigue_velo_gate_mlb_io.shuffle_tercile_planted_null)
-- MANDATORY, it MUST NOT beat BASE.
...
does NOT also pass this bar. Else REJECT. NOT_TESTABLE if the null replicates the real
lift (as wave-29 hit) -- this redesign's whole purpose is to test whether a REAL
pitcher-identity + REAL velocity proxy clears that bar where the weaker proxy could not.
INSUFFICIENT_DATA if no consolidated corpus is on disk.

NO $ anywhere; verdict is CALIBRATION (Brier), never a market edge. No pregame claim:
in-game conditioning ONLY. PROPOSAL-ONLY: writes only
data/domains/mlb/sp_velo_fatigue_gate_verdict.json, never data/registry/, flips no flag.
INVARIANTS: never edit src/ or kernel/; <=300 LOC; ASCII-only; numpy + stdlib.
CLI: python -m scripts.platformkit.ingame.ingame_sp_fatigue_velo_gate_mlb
"""
...
class FatigueVerdict:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "hypothesis_id": "H1_sp_fatigue_ingame_velo_redesign",
            "layer": "sp_within_start_velo_decline_tercile (last15-vs-first15 release_speed)",
            ...
            "redesign_of": "H1_sp_fatigue_ingame (wave-29, NOT_TESTABLE)",
...
def gate(states_by_game: Dict[str, List[dict]], order: List[str],
    ...
    """Run the real walk-forward gate + the MANDATORY planted-null control; return an
    honest SHIP/REJECT/NOT_TESTABLE FatigueVerdict."""
    ...
    # Planted-null control: IDENTICAL pipeline, tercile shuffled across REAL pitchers.
    ...
    planted_null = {
        "method": "shuffle velo-decline tercile across REAL pitcher within game-state cell",
    ...
    if null_ships:
        return FatigueVerdict("NOT_TESTABLE", scored["metrics"], scored["per_fold"],
                              ...
                              caveats + ["UNTRUSTWORTHY: planted-null (shuffled tercile) "
                                         ...
                                         "Per pre-registration: hypothesis class CLOSES."])
...
# Driver (corpus loading, CLI report, write) lives in
# ingame_sp_fatigue_velo_gate_mlb_driver.py to keep this module <=300 LOC.
