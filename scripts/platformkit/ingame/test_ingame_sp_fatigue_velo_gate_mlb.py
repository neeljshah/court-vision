"""Per-file test for ingame_sp_fatigue_velo_gate_mlb (+ its io/driver companions);
synthetic in-game states only, no parquet required (mirrors wave-29's
test_sp_fatigue_gate_mlb.py fixture classes for the redesigned velo-decline proxy).
...
from scripts.platformkit.ingame import ingame_sp_fatigue_velo_gate_mlb as G
from scripts.platformkit.ingame import ingame_sp_fatigue_velo_gate_mlb_io as IO


def _make_states(n_games: int, seed: int, tercile_signal: float = 0.0,
                 states_per_game: int = 60) -> list:
    """Synthetic per-pitch states. BASE driven by state_diff*frac (real signal);
    ...
    clean, time-independent conditioning signal a correct gate CAN detect). proxy_pitcher
    is a REAL-looking int id (mirrors the redesigned corpus's real Statcast identity)."""
    ...
    for g in range(n_games):
        gid = f"{seed}_{g}"
        pid = 100000 + seed * 1000 + g  # real-looking pitcher id, one per game/start
        ...
        for idx in range(states_per_game):
            ...
            out.append({"game_id": gid, "proxy_pitcher": pid,
...
def test_min_cell_fallback_keeps_beta_zero_for_sparse_tercile():
    ...
    from scripts.platformkit.ingame.ingame_gate_generic_models import fit_base, base_predict
    ab = fit_base(states)
    base_pred = base_predict(states, ab)
...
def test_shuffle_destroys_a_real_clean_signal():
    ...
    WEAKEN once the tercile assignment is shuffled within game-state cells."""
...
def test_verdict_dict_has_no_dollar_field_and_flags_calibration_only():
    ...
    assert d["hypothesis_id"] == "H1_sp_fatigue_ingame_velo_redesign"
    assert d["redesign_of"] == "H1_sp_fatigue_ingame (wave-29, NOT_TESTABLE)"
...
def test_real_on_disk_corpus_is_honestly_gated():
    """Smoke-check against the REAL on-disk sp_velo_states.parquet corpus (skips
    cleanly if absent, e.g. a fresh clone with no data/ dir): the gate must return one
    of the documented honest verdicts, never silently crash or fabricate a SHIP."""
    path = IO.find_corpus()
    if not path:
        return  # honest skip: data/ is local-only and gitignored
    from scripts.platformkit.ingame import ingame_sp_fatigue_velo_gate_mlb_driver as D
    v = D.run()
    assert v.verdict in {"SHIP_FATIGUE_LAYER", "REJECT", "INSUFFICIENT_DATA",
                        "INVALID_BASE", "NOT_TESTABLE"}
