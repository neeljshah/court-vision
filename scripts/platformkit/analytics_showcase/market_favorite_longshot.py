...
CONFOUNDS = [
    ...
    "this grades the closing price's own calibration, not our model -- nothing here is a claim about beating it",
]
# "forecast" is banned HERE on purpose: this exhibit grades the market's own
# prices, so the house phrase used elsewhere in the showcase would misname its
# subject. Say "closing price" in this module's prose.
...
def build() -> dict:
    ...
    return {
        ...
        "edge_claimed": False,
        "headline": (
            "The closing price is well-calibrated: MLB moneyline is essentially "
            "efficient, tennis shows only a mild favorite-longshot bias."
        ),
        "method": METHOD,
        "grades": "the closing price, not our model",
