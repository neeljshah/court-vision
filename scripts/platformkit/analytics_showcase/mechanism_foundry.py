"""One charged foundry trial per column-wired NBA mechanism.

Every mechanism in ``mechanism_wiring.WIRING`` becomes a row. A row with a
...
def build(rows: list[dict] | None = None, run_trials: bool = True) -> dict:
    """Turn prereg rows into result rows; idempotent given the same corpus."""
    rows = rows if rows is not None else prereg_rows()
    out = []
    for row in rows:
    ...
    return {"label": "DESCRIPTIVE_ONLY", "edge_claimed": False,
            ...
            "counts": {"mechanisms": len(out), "wired": len(out),
                       "with_trigger": len(wiring.TESTABLE),
                       "not_testable": len(out) - len(wiring.TESTABLE), "by_verdict": verdicts},
            "rows": out}
...
def main() -> int:
    ...
    rows = prereg_rows()
    PREREG_JSON.write_text(json.dumps(rows, indent=2, ensure_ascii=True), encoding="ascii")
    print("prereg written before any trial:", PREREG_JSON)
    result = build(rows)
