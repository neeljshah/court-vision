...
A6 self-monitor. The SINGLE document the UI panels (OpsPanel / ParityGrid / RatchetPanel)
and an operator runbook read to answer "is the autonomous system honestly working, and if
it is idle, WHY?". It composes -- never recomputes -- every observability primitive into
one envelope so no surface can read a dead feed as green.

WHAT IT COVERS (every value is NON-FABRICABLE; provenance per field below):
  * services[]   <- ops.health_aggregator.aggregate() VERBATIM (liveness + freshness SLA);
                    per-row severity is health_aggregator._row_severity (DEGRADE only).
  * loop         <- self-improve checkpoint (cycle, last_decision) + daemon status.jsonl
                    tail. A loop whose heartbeat is stale reads DEGRADED, not running.
  * ratchet      <- ratchet FSM checkpoint _ratchet_state.json (state + history). Pure read.
  ...
  * idle_reason  <- EXPLICIT honest "idle because X" derived from the daemon's OWN last
                    status row (no_new_games / no_candidate / cycle_done). Never a green blank.
...
monotone-DOWN only. A missing source yields DEGRADED with a reason, never absent-as-ok.
...
# Loop services gating the loop block, matched against services[].name; not OK / absent
# degrades it. ``m4_selfimprove`` is the canonical anchor (it IS this loop + stamps
# m4_selfimprove.txt every cycle): alive loop reads live, dead m4 stays degraded.
_LOOP_SERVICES = ("m4_selfimprove", "paper_loop", "ingame_live_loop", "predict_service")
...
def _services_block(health_doc: Dict[str, Any]) -> Dict[str, Any]:
    """Per-service rows + the WORST per-row severity (inherits _row_severity).

    aggregate() already merged liveness + freshness + breaker; we re-derive each row's
    severity with the SAME _row_severity the aggregator uses (stale feed -> RED here too),
    then take the worst. We never recompute liveness/freshness -- only read the verdict.
    """
...
def _loop_block(services: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    cycle/last_decision <- checkpoint.json; last_status <- status.jsonl tail. A loop SERVICE
    not OK in services[] forces this block off "running" (stale heartbeat never reads healthy).
    """
...
def _ratchet_block() -> Dict[str, Any]:
    ...
    _ratchet_state.json is written ONLY by the real improve.ratchet_state FSM (atomic). A
    missing file -> an honest IDLE shell with empty history, never a fabricated SHIP.
    """
...
def _high_water_block() -> Dict[str, Any]:
    ...
    The high_water key the daemon ACTUALLY advanced when it folded a settled game (self-
    improve checkpoint cursors). Nothing else writes it.
    """
    ckpt = _read_json(_IMPROVE_CKPT) or {}
    cursors = ckpt.get("cursors") if isinstance(ckpt.get("cursors"), dict) else {}
    out: Dict[str, Any] = {}
...
def _idle_reason(loop: Dict[str, Any], overall: str) -> Optional[str]:
    ...
    last_status = loop.get("last_status") or {}
    status = str(last_status.get("status") or "")
    if status == "no_candidate":
        # FIX D: no_candidate means NO CANDIDATE WAS BUILT (games declined PRE-gate -- inert,
        # cold start, degenerate base, or degraded feed). NOT "folded but the gate rejected"
        # (a gate rejection emits cycle_done w/ decision=REJECT).
        if last_status.get("armed") is True:
            return ("idle: no candidate built (settled games processed + declined "
                    "pre-gate: cold start / degenerate base)")
        return ("idle: no candidate built (self-improve inert -- PIPELINE_ENABLED "
                "sentinel absent; settled games preserved, not folded)")
    if status == "cycle_done":  # a gate VERDICT was actually reached (candidate built)
        dec = str(last_status.get("decision") or "")
        if dec == "REJECT":
            return "idle: candidate built, gate honestly REJECTED it (market efficient)"
        if dec == "REPLICATION_PENDING":
            return ("idle: candidate built + gate passed, awaiting a 2nd independent "
                    "corpus (not shipped on one corpus)")
        return None  # SHIP / other -> active work, not idle
    mapping = {
...
def compose(
    *,
    ...
    overall = WORST of health_aggregator's own overall, the worst per-service row severity,
    and the loop liveness severity. The composer can only DEGRADE (green never overrides).
    """
    ...
    return {
        ...
        "notes": notes,
        "honest_note": ("calibration, not edge; no $ field. A stale/dead feed is RED, "
                        "never green; an idle system says WHY it is idle."),
    }
