...
@mock.patch("scripts.platformkit.answers.qa_runner.run_smoke_job",
            return_value={"n_pass": 15, "n_fail": 0, "n_run": 15})
...
def test_run_all_isolates_one_failing_job(_cg_mock, _av_mock, _econ_mock, _qa_mock, tmp_path):
    # card_grade/analytics_verify/econ_scoreboard_refresh/qa_bank_smoke mocked via
    # decorator: the with-stack below is already at CPython's 20-nested-block cap,
    # one more with-item is a SyntaxError.
    ...
    assert out["qa_bank_smoke"] == {"n_pass": 15, "n_fail": 0, "n_run": 15}
