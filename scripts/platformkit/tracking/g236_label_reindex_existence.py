"""Run G236's bounded, memory-only labelled-still existence search on the pod."""
...
LABELLED_INDEX = 28171
COARSE_SIZE = (64, 36)
COARSE_STRIDE = 5


def _worker_source(label_b64: str) -> str:
    """Return the stdin-only pod worker; it writes neither frames nor source files."""
...
if label is None or label.shape != (360, 640, 3):
    raise RuntimeError("expected 640x360 BGR label still")
...
while True:
    ...
    if decoded % COARSE_STRIDE == 0:
        ...
        distance = float(np.abs(small.astype(np.int16) - target_small.astype(np.int16)).mean())
        distances.append([decoded, distance])
    decoded += 1
capture.release()
if not distances:
...
candidate_640 = cv2.resize(candidate_native, (640, 360), interpolation=cv2.INTER_AREA)
confirmation_mad = float(np.abs(candidate_640.astype(np.int16) - label.astype(np.int16)).mean())
baseline_native = raw_exact_range(LABELLED_INDEX, LABELLED_INDEX, width, height)[0]
baseline_640 = cv2.resize(baseline_native, (640, 360), interpolation=cv2.INTER_AREA)
baseline_mad = float(np.abs(baseline_640.astype(np.int16) - label.astype(np.int16)).mean())
encoded_ok, encoded_candidate = cv2.imencode(".jpg", candidate_640,
...
report = {{
    ...
    "decode_method": "sequential cv2.VideoCapture.read from frame 0 through EOF; refinement and MAD use ffmpeg select=eq(n,index)",
    ...
    "confirmation_mad_640x360": confirmation_mad,
    "labelled_index_baseline_mad_640x360": baseline_mad,
    "wall_seconds": time.perf_counter() - started,
    "disk_guard": {{"pre_du_mb": int(os.environ["G236_PRE_DU"]),
                   "post_du_mb": int(os.environ["G236_POST_DU"]),
                   "probe_bytes": int(os.environ["G236_PROBE_BYTES"]),
                   "probe": "dd 4MiB conv=fsync passed then removed"}},
    "code_identity": {{
        "worker_sha256": os.environ["G236_WORKER_SHA256"],
        "opencv": cv2.__version__, "ffmpeg_path": ffmpeg_path,
        "ffmpeg_sha256": sha256_path(ffmpeg_path) if ffmpeg_path else None,
    }},
    "candidate_640_jpeg_b64": base64.b64encode(encoded_candidate.tobytes()).decode("ascii"),
}}
...
def remote_script(label_b64: str) -> str:
    """Build the guarded pod command; only the required probe is written then removed."""
...
PROBE="$DATA_ROOT/.g236_label_reindex_probe_$$"
...
printf '%s' '{worker}' | base64 -d | env G236_WORKER_SHA256={worker_sha256} G236_PRE_DU="$PRE_DU" G236_POST_DU="$POST_DU" G236_PROBE_BYTES="$PROBE_BYTES" python3 -
'''


def run(still: Path, output_json: Path, candidate_image: Path,
        comparison_image: Path, ssh_config: Path, ssh_host: str) -> dict:
    """Run the search, preserve recomputable evidence, and create final local images."""
    ...
    report["label_source"] = {
        "path": str(still.resolve()), "bytes": len(label_bytes),
        "sha256": hashlib.sha256(label_bytes).hexdigest(), "resolution": [640, 360],
    }
    report["launcher_sha256"] = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    candidate_jpeg = base64.b64decode(report.pop("candidate_640_jpeg_b64"))
    candidate_array = np.frombuffer(candidate_jpeg, dtype=np.uint8)
    candidate = cv2.imdecode(candidate_array, cv2.IMREAD_COLOR)
    ...
    comparison = cv2.hconcat([label, candidate])
    if not cv2.imwrite(str(comparison_image), comparison):
...
def main() -> None:
    ...
    parser.add_argument("--ssh-host", default="config.pod")
    args = parser.parse_args()
    report = run(args.still, args.output_json, args.candidate_image,
                 args.comparison_image, args.ssh_config, args.ssh_host)
    print("G236_BEST_FRAME=" + str(report["best"]["frame_index"]))
    print("G236_BEST_DISTANCE=" + f'{report["best"]["distance"]:.6f}')
