...
# G296B adaptation: ships only explicit --ship files from this worktree to /workspace/wt/<aN> on the pod, links data/ to the deployed tree's data (read-only use), runs the
...
set -u
H="$1"; shift
WT="/c/Users/neelj/nba-track-$H"; [ "$H" = a12 ] && [ -d "$WT" ] || exit 2
...
$SSH "mkdir -p $R && dd if=/dev/zero of=$R/g296b_launcher_fsync_probe bs=1M count=8 conv=fsync" || { echo "FAILED dd conv=fsync probe"; exit 3; }
echo "POD /workspace usage UNKNOWN (network walk omitted); no size stop. CPU gate is in extraction harness."

# Ship only explicit paths; no full Python-tree transfer. tar exit 1 is tolerated as upstream.
( cd "$WT" && { for x in "${SHIP[@]}"; do [ -e "$x" ] && { [ -d "$x" ] && find "$x" -type f || echo "$x"; }; done; } | tar -cf - -T - 2>/dev/null ) | $SSH "cd $R && tar -x --no-same-owner" ; rc=${PIPESTATUS[0]}; [ "$rc" -le 1 ] || { echo "SHIP FAILED rc=$rc"; exit 4; }
