...
# enumeration below is 31, including both the frontend and domains tennis
# surface-hold verdict pair, which are distinctly-shaped files, plus the
# PROGRAM v3 fit-validity-gate verdict added in the fit-verdict-into-ask lane).
ALL_FILES = [
    ...
    "data/domains/nba/fit_validity_gate_verdict.json",
...
NEW_CLAIMS = [
    ...
    new_claim(
        "nba_fit_validity_gate_verdict",
        "Does scheme fit predict post-move performance? (NBA fit-validity gate, V2 spec)",
        "fit_validity_gate (domains.nba, V2 spec sha16 2df32a291c3190db, H0 plain-base vs H1 fit-conditioned prior RMSE across 417 historical player-moves, 5 season folds)",
        "data/domains/nba/fit_validity_gate_verdict.json",
        "REJECT",
        -0.06179056543743453,
        ["nba_historical_moves_2020_2025"],
        True,
        {
            "verdict": "verdict",
            "primary_number": "pooled_delta",
            "planted_null_passed": "null1_result.dies",
        },
        [
            "pooled_delta=-0.0618 (H1 fit-conditioned prior does NOT beat H0 plain base on pooled RMSE across all 417 moves/5 folds); sign_holds='1/5' -- the H1-beats-H0 direction only held in 1 of 5 season folds, not a directionally consistent effect.",
            "planted_null_passed cites null1_result.dies=True; a SECOND independent planted null (null2_result.dies=True) also died -- both noise controls behaved as expected (neither shuffled-null variant spuriously beat H0 either), so the REJECT is not an artifact of a broken null design.",
            "This is the pre-registered V2 spec (sha16 2df32a291c3190db); the verdict json (data/domains/nba/fit_validity_gate_verdict.json) is the sole read-only source of truth and is never regenerated by this claim.",
            "CALIBRATION only (held-out RMSE on post-move performance), never a market/$ edge. Answers the scouting-composition validity question honestly: composition (compose_fit) is DESCRIPTIVE only -- this gate is the pre-registered check of whether that description predicts anything, and it does not.",
        ],
    ),
    new_claim(
