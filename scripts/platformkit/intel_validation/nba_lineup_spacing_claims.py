...
def build_spacing_claim() -> dict[str, Any]:
    df = pd.read_parquet(_SPACING_SRC)
    n_considered = len(df)
    ...
    return {
        "claim_id": f"nba_lineup_spacing_full_{SEASON_WINDOW}",
        "kind": "ranking",
        "question": (
            ...
            f"{SEASON_WINDOW} 196-game slice)?"
        ),
        "criteria": {
        ...
        "ranking": ranking,
        "source_files": [_rel(_SPACING_SRC)],
        ...
        "caveats": [
            ...
            "the floor, from data/cache/team_system/lineups/lineup_spacing_2025_26.parquet "
            "(pbp_lineups.py's reconstructed stints, 196-game 2025-26 PBP corpus). A "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Emit NBA lineup-spacing ranking claim")
    parser.add_argument("--output", type=str, default=str(_CLAIMS_OUT))
    args = parser.parse_args(argv)

    claim = build_spacing_claim()
