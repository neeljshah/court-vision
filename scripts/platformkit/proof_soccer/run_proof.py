"""scripts.platformkit.proof_soccer.run_proof — CLI entry point for the soccer proof.
...
EXPECTED VERDICTS (written before any gate run — the honest discipline):
  soccer_rest_congestion → REJECT  (fixture congestion is fully public and priced)
  soccer_totals_form     → REJECT  ('over team' narrative stat, priced by totals market)
  soccer_h2h_totals      → REJECT  (H2H narrative stat, sparse samples, noise-dominated)
A REJECT is the success criterion — it proves the gate's honesty transfers across sports.

Market-efficiency framing (binding):
  Football-data O/U 2.5 markets are efficient.  Devigged closing-line Brier EXPECTED
  to beat the calibrated Poisson model on every eval corpus.  V1 success = calibration
  quality + plumbing, not edge over the market.  All 3 signals expected REJECT;
  that demonstrates kernel honesty transfers to sport-3 (soccer).

F5 compliance: ZERO domains.nba / domains.basketball_nba / other-sport-domain /
src.data / src.sim / src.tracking / src.pipeline imports.

CLI:  python run_proof.py --corpus data/domains/soccer [--report PATH]
If corpus parquets are absent, exits cleanly (code 2) — no crash.

PRIVATE: report is price-bearing; default path .planning/platform/proof_soccer/
...
from scripts.platformkit.proof_soccer.proof_runner import run_v1, run_v2, run_v3, run_v4

logger = logging.getLogger(__name__)

_DEFAULT_REPORT = ".planning/platform/proof_soccer/PROOF_RESULT.md"
...
def _load_adapter(corpus_dir: Path) -> Optional[SoccerAdapter]:
    """Return a SoccerAdapter or None when matches.parquet is absent."""
...
def _fmt_v1(v1: Dict[str, Any]) -> str:
    ...
    for label, corpus in v1.get("detail", {}).items():
        ...
        lines += [
            ...
            f"  market_beats_model : {corpus.get('market_beats_model')}  <- EXPECTED yes",
...
def _fmt_v3(v3: Dict[str, Any]) -> str:
    lines = [
        ...
        "Pre-run expected verdicts (SOCCER_PROOF_PLAN.md §4.4):",
        "  soccer_rest_congestion → REJECT  (fixture congestion fully priced)",
        "  soccer_totals_form     → REJECT  (over-team narrative stat, priced)",
        "  soccer_h2h_totals      → REJECT  (H2H narrative stat, sparse samples)",
        "",
...
def write_report(report_path: Path, v1: Dict, v2: Dict, v3: Dict, run_ts: str,
    ...
    body = textwrap.dedent(f"""\
        # Soccer Third-Domain Proof — Results

        > **PRIVATE** — price-bearing; never commit to public repo.
        > Generated: {run_ts}
        > Proof spec: SOCCER_PROOF_PLAN.md  |  F5: ZERO other-sport-domain imports.
        ...
        **Overall: {overall}**
        ...
        - [ ] F1. No kernel file needed a logic edit (hash delta) to run soccer.
        - [ ] F2. No kernel function has a soccer-conditional branch.
        - [x] F3. gate.evaluate ran on sport-3 data via injected FeatureBundle (V3).
        ...
        - [ ] F6. Adapter LOC within budget.

        ## Market-efficiency framing

        Football-data O/U 2.5 markets are efficient. Expected: devigged closing-line
        Brier < calibrated Poisson model Brier (market beats us). V1 records this
        honestly — V1 success is calibration quality and plumbing, not edge over the
        market. All 3 signals expected REJECT; that demonstrates kernel honesty
        transfers to sport-3 (soccer).
        """)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Soccer third-domain proof runner (V1/V2/V3/V4)."
    )
    parser.add_argument("--corpus", default="data/domains/soccer")
    ...
    if adapter is None:
        print(
            ...
            "Run domains/soccer/ingest_footballdata.py first.",
            file=sys.stderr,
        )
        return 2
    ...
    paper_book_dir = (
        ...
        else repo_root / "data" / "domains" / "soccer" / "paper_book"
    )

    run_ts = dt.datetime.utcnow().isoformat(timespec="seconds") + "Z"
    print(f"[run_proof] Starting soccer proof at {run_ts}")
