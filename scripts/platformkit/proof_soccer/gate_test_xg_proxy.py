"""scripts.platformkit.proof_soccer.gate_test_xg_proxy -- HONEST as-of xG-proxy gate test.

Threads the leak-free as-of shots-based xG-PROXY diffs (domains.soccer.asof_xg_proxy)
through the REAL honest gate (src.loop.gate.evaluate) on top of the soccer base bundle.

REUSES the proven base-bundle construction from gate_test_asof (_build_base_bundle_with_ids,
_align_asof, _ArraySignal); only the candidate columns + parquet differ.

DISCIPLINE (binding): expected verdict REJECT or DEFER -- a team's shot-based expected
output is largely team STRENGTH, already priced into the rating base. A SHIP is a PROBABLE
ARTIFACT; NO edge is ever claimed. "xg" is an explicit shots PROXY (no true xG on disk).
"""
...
_ASOF_PARQUET = _REPO_ROOT / "data" / "domains" / "soccer" / "asof_xg_proxy.parquet"

_PREFERRED_CANDIDATES: Tuple[str, ...] = (
    "diff_xg_for_asof",
    "diff_xg_against_asof",
    "diff_xg_supremacy_asof",
)
...
def run_gate_test(
    ...
    """Run each candidate xG-proxy diff through the REAL gate; return verdict rows."""
    apath = Path(asof_path) if asof_path is not None else _ASOF_PARQUET
    if not apath.exists():
        raise FileNotFoundError(
            f"asof_xg_proxy.parquet not found at {apath}. Run "
            "domains.soccer.asof_xg_proxy first.")
    asof_df = pd.read_parquet(apath)

    bb, event_ids = _build_base_bundle_with_ids(seasons=seasons)
    n = bb.base.shape[0]
    rows: List[dict] = []
    for col in _candidate_columns(asof_df):
        ...
        if verdict == "SHIP":
            logger.warning(
                "SOCCER xG-PROXY SHIP FLAG '%s': PROBABLE ARTIFACT. NO edge claimed.", col)
        rows.append({
...
def _summary_line(rows: List[dict]) -> str:
    if not rows:
        return "Soccer as-of xG-proxy: NO candidate columns evaluable -> DEFER; no edge."
    ships = [r["name"] for r in rows if r["verdict"] == "SHIP"]
    verds = sorted({r["verdict"] for r in rows})
    if ships:
        return ("Soccer as-of xG-proxy: SHIP flagged for " + ", ".join(ships) +
                " -> PROBABLE ARTIFACT, NO edge claimed (artifact-hunt required).")
    return ("Soccer as-of xG-proxy: " + "/".join(verds) +
            " -> NO edge; shot-based output is team strength, already priced. Calibration only.")


def main() -> int:
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    rows = run_gate_test()
    hdr = f"{'Candidate':<32} {'Verdict':<8} {'Cover':>6} {'wf_all':>7} {'abl':>5} {'null':>5} {'calib':>6} {'p':>8}"
    print("\nSoccer as-of xG-PROXY gate test (REAL gate)")
    print("=" * len(hdr))
    print(hdr)
    print("-" * len(hdr))
    for r in rows:
        p = r["p_value"]
        print(f"{r['name']:<32} {r['verdict']:<8} {r['coverage']:>6.3f} "
