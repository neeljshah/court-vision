...
LANE 4b: order-book DEPTH accrual asset (value compounds the earlier it starts).
Live-verified recipe (memory ingame_data_sources_2026_07_04, item 5): Kalshi's
public KEYLESS orderbook endpoint

    GET /trade-api/v2/markets/{ticker}/orderbook

returns {"orderbook_fp": {"yes_dollars": [[price_str, size_str], ...],
"no_dollars": [[price_str, size_str], ...]}} -- each a price-ladder rung list,
NOT a single best bid/ask. DATA CAPTURE ONLY: no model, no gate, no edge
framing. Snapshots the FULL ladder (not just top-of-book) for every in-play
market the existing widened in-play capture already tracks
(kalshi_series_spec.SERIES_SPEC, the same series list inplay_kalshi iterates),
append-only JSONL, one row per (ticker, snapshot).

REUSE, never duplicate: kalshi_series_spec.series_for (series list) +
kalshi._BASE (base URL) + kalshi_pacing (inter-request stagger + 429
cool-down/observability, same politeness discipline inplay_kalshi uses) +
transport.resilient_get_json (escalating HTTP transport) + live_grade.
_append_atomic's tmp+os.replace discipline (mirrored here, not imported, to
keep this module import-independent of the ingame/ package).

Row shape (append-only JSONL, one line per ticker per snapshot):
  {"ts": iso8601Z, "sport": str, "ticker": str, "event_ticker": str,
   "yes_bids": [[price, size], ...], "yes_asks": [[price, size], ...],
   "depth_totals": {"yes_bid_total": float, "yes_ask_total": float},
   "source": "kalshi", "source_url": str, "fetched_at": iso8601Z,
   "capture_version": 1}
*fetched_at* is the TRUE network-fetch time (never a recomputed now()); *ts*
is the snapshot-cycle time shared across every ticker in one pass -- distinct
so a lagged per-ticker fetch is still honestly visible.

An orderbook fetch failure (network, 429, malformed body) for ONE ticker is
skipped (never fabricated/zero-filled), isolated per-ticker like
inplay_kalshi._fetch_one_series.
...
def fetch_orderbook_row(sport: str, ticker: str, event_ticker: str, ts: str, *,
                         ...
                         sleep_fn: SleepFn = time.sleep) -> Optional[Row]:
    ...
    n_requests/n_429 convention kalshi_pacing uses elsewhere. Never raises."""
...
def _list_tickers(sport: str, *, http: HttpGet, stats: Optional[Dict[str, Any]],
                   sleep_fn: SleepFn, stagger_sec: float) -> List[Dict[str, str]]:
...
def capture_depth_snapshot(sport: str, *, http: HttpGet = resilient_get_json,
                            ...
                            max_tickers: Optional[int] = None) -> List[Row]:
    """ONE depth snapshot pass across every active market in *sport*'s wired
    Kalshi series: list active tickers then fetch ONE orderbook row per
    ticker. *stats* aggregates n_requests/n_429 across listing + per-ticker
    calls. *max_tickers* bounds a pass (None -> no cap; a live daemon should
    bound it so one sport's slate can never starve the cycle's cadence).
    Never raises: an unsupported sport or dead feed yields []."""
...
def append_rows_atomic(rows: List[Row], path: Path) -> int:
    """Append *rows* (one JSON dict per line) to *path* via tmp + os.replace
    (same atomic-rewrite discipline as live_grade._append_atomic) so a reader
    never sees a partial line. Returns the count actually written; 0 on a
    write failure (logged) -- callers must not treat 0 as "nothing to write"
    without also checking whether rows was non-empty."""
    if not rows:
        return 0
    try:
...
def run_capture_pass(sports: List[str], *, http: HttpGet = resilient_get_json,
    ...
    per sport, dated by the pass's UTC date. Returns {"sports": {sport:
    {"n_markets","n_rows_written","path"}}, "ts", "n_requests_total",
    "n_429_total"}. Never raises: a per-sport failure isolates (0 rows,
    logged)."""
    ts = now_iso or _utc_iso()
    dated = ts[:10]
    ...
    for sport in sports:
        ...
        try:
            rows = capture_depth_snapshot(
                ...
                max_tickers=max_tickers_per_sport)
