"""domains.basketball_nba.states_gate -- LANE 2 NBA states gate (core math),
PORTED from domains.basketball_wnba.states_gate.

PRE-DECLARED QUESTION: does run_last_3min / in_bonus, each as a SINGLE
logit-additive term on a frozen anchored p, improve checkpoint Brier on the
NBA CDN checkpoint-states corpus (lane 4 backfills it) joined to
linescores, cross-fitted md5 halves (fit h0 eval h1, and vice versa)? One
coef per feature, no interactions:  p_test = sigmoid(logit(p_anchored) +
coef*feature). feature = run_last_3min (raw) or in_bonus_diff in {-1,0,1}.
coef is a coarse deterministic 1-D grid search (WNBA gate's style)
minimizing pooled Brier on the FIT half, scored on EVAL; coef=0 always
in-grid. THE ANCHORED p IS NEVER REFIT -- only an additive term on top is
evaluated; adoption is a future human-queue item, not automatic.

NBA vs WNBA: regulation = 2880s (4x12min) vs WNBA's 2400s (4x10min);
checkpoint TIMES parameterized via REG_SEC/PERIOD_SEC (_CHECKPOINT_PERIOD).
No closed-form blend_prob(p0, LiveState)/ANCHORED_K exists for NBA yet
(ingame_blend_surface.py fits a WeightSurface, not a frozen closed form) --
this module takes p_anchored as an ALREADY-COMPUTED column
(states_gate_runner.py wires it, currently p_home_elo passthrough);
states_gate.py only reads row.p_anchored, agnostic to its source.

CROSS-FIT (leak-free): 2 halves by md5(str(event_id)) parity (no RNG). Fit
h0 eval h1 (A); fit h1 eval h0 (B) -- neither fit sees its own eval rows.
HONEST FLOORS: below _MIN_N_PER_HALF a cell is INSUFFICIENT, never SHIP/
REJECT. The NBA checkpoint-states corpus does not exist on disk yet (lane 4
backfills it) -- see states_gate_join.py / states_gate_runner.py for the
honest NO_CORPUS path.

PROVENANCE: any NBA cdn checkpoint-states corpus is VALIDATION, joined to
linescores.parquet ONLY for this check -- never pooled with a forward-only
capture stream, per AUTONOMY_CHARTER. This module = Row/build_rows/
fit_coef/crossfit/descriptive_stats (pure computation); states_gate_runner.py
assembles the on-disk gate; states_gate_ci.py adds bootstrap CIs.

INVARIANTS: <=300 LOC; ASCII only; pandas + stdlib only; no network; no
$/roi/edge fields; never mutates ingame_blend_surface.py or ratings.py.
Test: cd /c/Users/neelj/nba-ai-system && python -m pytest domains/basketball_nba/test_states_gate.py -q
"""
...
# NBA regulation: 4 x 12min quarters = 2880s (vs WNBA's 4 x 10min = 2400s).
REG_SEC: float = 2880.0
PERIOD_SEC: float = 720.0
...
# Honest floor: a checkpoint-half cell below this n is INSUFFICIENT, never
# scored SHIP/REJECT (CIs at n<40 games are not decision-grade).
_MIN_N_PER_HALF = 40

# Coefficient grid: fixed, deterministic, 0.0 always included (no-term case).
# run_last_3min is raw point-margin scale (~[-15,15]); in_bonus_diff in {-1,0,1}.
...
def _checkpoint_diff(ls: pd.Series, period: int) -> float:
    """NBA linescores schema is home_q1..home_q4 (per-quarter, not
    cumulative) -- sum quarters up to `period` for the cumulative
    score-diff at that checkpoint (WNBA's cols are already cumulative)."""
    home = sum(float(ls[f"home_q{q}"]) for q in range(1, period + 1))
    away = sum(float(ls[f"away_q{q}"]) for q in range(1, period + 1))
    return home - away
...
def build_rows(
    ...
    games_with_p0 must carry columns: event_id, p_anchored, home_win --
    p_anchored is ALREADY-COMPUTED (states_gate_runner.py produces it)."""
    ...
    for gid, event_id in game_id_to_event_id.items():
        ...
        for cp in CHECKPOINTS:
            ...
            # p_anchored is passed through as given (score-agnostic here);
            # see module docstring + states_gate_runner.py for the source.
...
__all__ = [
    "REG_SEC", "PERIOD_SEC", "CHECKPOINTS", "FEATURES", "_MIN_N_PER_HALF",
    "Row", "CheckpointCrossfitResult", "half_of", "build_rows", "fit_coef",
    "crossfit_checkpoint_feature", "descriptive_stats"]
