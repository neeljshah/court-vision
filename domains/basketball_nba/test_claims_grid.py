"""Drift-guard + grammar-parse tests for domains/basketball_nba/claims_grid.py.
...
Run: python -m pytest domains/basketball_nba/test_claims_grid.py -q
"""
...
from domains.basketball_nba.claims_grid import GRID
...
def test_grid_shape_sane() -> None:
    assert GRID["sport"] == "basketball_nba"
...
def test_every_window_type_has_a_required_floor() -> None:
    ...
    for fam in GRID["families"]:
        ...
        for window in fam["windows"]:
            name = window["name"]
            # season_* and career_to_date map to "season"/"career"; context
            # splits (declared separately) map to "split". Every window name
            # in this grid is one of the two window-type buckets.
...
def test_grid_module_has_zero_imports_beyond_future() -> None:
    ...
    src = (REPO_ROOT / "domains" / "basketball_nba" / "claims_grid.py").read_text(encoding="ascii")
