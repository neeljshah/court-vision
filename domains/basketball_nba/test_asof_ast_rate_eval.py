"""Per-file test for domains.basketball_nba.asof_ast_rate_eval.

Acceptance criteria
-------------------
1. build_candidate_frame (synthetic, no real-corpus touch):
   a. Left-merges ast_rate_diff_asof keyed on game_id; missing asof row -> NaN
...
3. _planted_null: a shuffled feature collapses -> verdict REJECT.

Run:
    cd /c/Users/neelj/nba-ai-system && python -m pytest \
        domains/basketball_nba/test_asof_ast_rate_eval.py -q
"""
...
from domains.basketball_nba import asof_ast_rate_eval as mod
...
def _synthetic() -> tuple:
    ...
    n = 400
    dates = pd.date_range("2015-10-20", periods=n, freq="D")
    teams = [f"T{i % 8}" for i in range(n)]
    aways = [f"T{(i + 3) % 8}" for i in range(n)]
    gids = [f"00{i:08d}" for i in range(n)]
    hw = rng.integers(0, 2, n).astype(float)
    games = pd.DataFrame({
        "game_id": gids, "date": dates, "season": "2015-16",
        "home_team": teams, "away_team": aways, "home_win": hw,
    })
    # asof sidecar: drop the last row (absent) -> merge must yield NaN + _cov False
    asof = pd.DataFrame({
        "game_id": gids[:-1],
        "ast_rate_diff_asof": rng.normal(0, 0.05, n - 1),
...
def test_build_candidate_frame_merge_and_nan():
    ...
    for c in ("game_id", "home_win", "p_base", "ast_rate_diff_asof", "_cov"):
        assert c in df.columns
    assert df["date"].is_monotonic_increasing
    last = df.iloc[-1]
    assert pd.isna(last["ast_rate_diff_asof"])
