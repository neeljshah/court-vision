...
- **status**: CONFIRMED (REPLICATED -- 3-season box-detail backfill, 2026-07-11)
- **measured LOCAL magnitude (original, 2026 slice)**: persistence r=0.779, p=3.97e-7 (n=30 teams); margin relation r=0.845, p~0 (n=1,342 team-games).
- **replication (2026-07-11, Q8/WEEKEND_PLAN section 4)**: 2 disjoint season corpora landed (`espn_boxscores_2024_25.parquet`, 1,229 non-null; `espn_boxscores_2023_24.parquet`, 1,230 non-null, brand-new that day) -- largest_lead was the one box-detail row #34/#35/#44's 2026-07-10 PM wave left un-replicated. Both new seasons individually clear the original bar (ALPHA=0.01, MIN_EFFECT=0.15), same sign both legs: 2024-25 persist r=0.6967 p=1.89e-5, margin r=0.813 p~0 (n=2,458); 2023-24 persist r=0.8021 p=9.83e-8, margin r=0.8138 p~0 (n=2,460). Pooled 3-season + Fisher-combined (pre-registered rule: >=2/3 sign-consistent + Fisher p<0.01 + pooled CI excludes zero, both legs): persist Fisher p=6.76e-16, pooled r=0.4587, CI95[0.1179,0.7028]; margin Fisher p~0, pooled r=0.821, CI95[0.8128,0.8289]. Both legs PASS -> per the #34/#35 family convention (disjoint-season box-detail corpora count as independent corpora for promotion), label upgraded CONFIRMED_LOCAL -> REPLICATED.
- **artifact link**: `domains/basketball_nba/knowledge/validate_boxdetail_persistence.py::hypothesis("largest_lead")` (original); `scripts/platformkit/improve/q8_largest_lead_pool.py` (replication + pool + Fisher, this session); `domains/basketball_nba/knowledge/validation_ledger.jsonl`, hypothesis=`boxdetail_largest_lead_persistence_q8_*`.
- **wiring**: candidate for the interaction factory's box_detail family (`boxdetail_asof.py`'s `largest_lead_diff_asof`); still calibration/mechanism knowledge only, not gated for a pregame outcome edge -- no edge claimed.
...
- **test spec**: `domains.basketball_nba.knowledge.validate_research_wave5.lineup_continuity_streak_vs_point_diff` -- per (game_id,team_id) opening lineup_key = first `period==1` stint by `start_s`; sort each team's games by `game_id` (chronological proxy, same convention used elsewhere in this repo); continuity_streak = count of consecutive prior games with identical opening lineup_key; Pearson r, streak_length vs point_diff; declared bar |r|>=0.05 AND p<0.01 (large-n floor, ~2,460 rows/season); 2023-24 vs 2024-25 as the two independent-corpora replication legs (2025-26, in progress at 1,192/1,230 games, held out).
- **status**: CONFIRMED_LOCAL -- both independent-corpora legs clear the declared bar, same sign (per the row's own confound flag: read as a health/availability proxy, not a chemistry claim, until an injury-report control is added).
- **measured LOCAL magnitude**: 2023-24 r=0.0535, p=0.00798, n=2,460 team-games; 2024-25 r=0.0788, p=9.05e-05, n=2,460 team-games -- both clear |r|>=0.05 AND p<0.01, same (positive) sign.
- **artifact link**: `domains/basketball_nba/knowledge/validate_research_wave5.py::lineup_continuity_streak_vs_point_diff`; `domains/basketball_nba/knowledge/validation_ledger.jsonl`, hypothesis=`lineup_continuity_streak_vs_point_diff__2023-24`/`__2024-25`/`__combined`.
