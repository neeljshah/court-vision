...
### 42. Team staff-wide high-pitch-count day precedes next-day run-prevention degradation -- CONFIRMED_LOCAL (2026-07-11 update, was PROVISIONAL)
...
- **status (original 2026-07-10 per-season test)**: 2023: NULL_LOCAL (eff=+0.079, p=0.580, n=2,087); 2024: CONFIRMED_LOCAL (eff=+0.392, p=0.0043, n=2,160); 2025: NULL_LOCAL (eff=+0.327, p=0.023, misses alpha=0.01, n=2,078). All three seasons point the same (support) direction but only one cleared significance per-season -- flagged PROVISIONAL and queued (WEEKEND_WATCHBOARD D1) to check whether the per-season split was simply underpowered.
- **2026-07-11 D1 resolution -- pooled + Fisher-combined, pre-registered rule (Fisher p<0.01 AND pooled effect 95% CI excludes zero AND sign-consistent >=2/3 seasons -> CONFIRMED_LOCAL)**: pooled-3 (2023-2025, same leak-audit/quartile-split construction, n=6,407 pairs) effect=+0.2729 runs, 95% CI [+0.1147, +0.4311] (excludes zero); Fisher combine of the 3 per-season p-values [0.580, 0.004276, 0.02323] -> chi2=19.524, p=0.003365 (< alpha=0.01); sign-consistent 3/3 seasons -> **upgrade to CONFIRMED_LOCAL**. Interpretation: the per-season split WAS underpowered at n~2,100/season and alpha=0.01 -- pooling recovers a significant, sign-consistent, CI-excludes-zero effect. Single corpus family (Statcast/savant only) -- this is CONFIRMED_LOCAL, explicitly NOT "REPLICATED" (would need an independent data source, e.g. a different play-by-play provider).
- **2026-07-11 4th-season OOS check (2026 partial season, NOT part of the original 3-season test, kept separate)**: 2026 alone: NULL_LOCAL (eff=+0.191, p=0.321, n=1,210, partial season through data-cache cutoff -- likely underpowered at this n, not a sign-flip). Fisher combine of all 4 seasons -> chi2=21.797, p=0.005306 (<0.01); pooled-4 effect=+0.2600, 95% CI [+0.1122, +0.4078] (excludes zero), sign-consistent 4/4. The 2026 OOS season does not overturn the finding -- same direction, just individually underpowered.
- **measured LOCAL magnitude**: see above (savant_full__2023-2025 pooled + 2023-2026 Fisher-combined).
- **artifact link**: `domains/mlb/knowledge/validate_staff_dayafter_chain.py::run` (original per-season); `scripts/platformkit/improve/d1_staff_dayafter_pool.py::main` (pooling + Fisher combine, reuses `team_day_table`/`build_dayafter_pairs`/`_compare` unchanged); `validation_ledger.jsonl` rows `staff_dayafter_fatigue_chain` (x3) + `__combined` + `__fisher3` + `__oos_2026` + `__fisher4_incl_2026oos`.
