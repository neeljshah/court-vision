...
- **test spec**: `domains.mlb.knowledge.validate_research_wave3.pinch_sub_platoon_targeting` (not yet built) -- per team-game-slot, identify the substitute (lower-PA player sharing a `batting_order` slot, `player_gamelogs.parquet`), join first PA to `savant_full__2025.parquet` via `batter`==`player_id` for `stand`/`p_throws`; one-sample proportion test of substitute same-hand rate vs the season-wide baseline same-hand PA rate; declared bar |eff|>=0.05 (rate points) AND p<0.01.
