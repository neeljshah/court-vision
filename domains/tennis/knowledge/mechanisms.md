...
### 39. Within-tournament ace rate rises from early to late rounds (surface-speed-drift proxy, sidesteps #36's date-granularity gap by using round instead) -- LOCAL NULL
...
- **premise check (hard, per instruction)**: `data/domains/tennis/_raw/sackmann_pbp_repos/tennis_MatchChartingProject/charting-m-stats-ReturnDepth.csv` confirmed this session -- columns `match_id`/`player`/`row`/`returnable`/`shallow`/`deep`/`very_deep` (+ error-breakdown cols); `row`=='Total' subset: 15,115 player-match rows across 7,545 matches, 1,002 distinct players. **Implementation note**: every `match_id` carries an embedded YYYYMMDD date prefix (verified 100% on the Total subset) -- used directly for split-half date ordering instead of the separate join to `charting-m-matches.csv`'s `Date` column the row named (same information, one fewer file read).
...
- **test spec**: `domains.tennis.knowledge.validate_research_wave5.return_depth_aggressiveness_persistence` -- deep_rate = (deep+very_deep)/returnable per player-match (Total row only); split by match date (first half of date range vs second), floor >=5 matches/half & >=15 players (matches #18/#30 house floor convention); split-half Pearson r; declared bar |r|>=0.15 AND p<0.01 (matches #30/#32 bar).
- **status**: REJECTED (NULL_LOCAL) -- raw split-half correlation neither clears |r|>=0.15 nor p<0.01, echoing #30/#32's "no independent skill/style persistence" pattern on this corpus.
- **measured LOCAL magnitude**: split-half pearson r=0.1348, p=0.249, n=75 players (>=5 matches/half in BOTH date-halves).
- **artifact link**: `domains/tennis/knowledge/validate_research_wave5.py::return_depth_aggressiveness_persistence`; `domains/tennis/knowledge/validation_ledger.jsonl`, hypothesis=`return_depth_aggressiveness_persistence`.
