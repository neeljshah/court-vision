...
### 37. Breaking serve elevates the breaker's hold rate in the immediately following game (game-level momentum, distinct from #16's point-level streak effect)
...
### 38. New-ball cycle position predicts serve execution (ace rate / serve speed)
...
- **test spec**: `domains.tennis.knowledge.validate_research_wave3.ball_cycle_serve_execution` (not yet built) -- derive per-match cumulative game index from `slam_points.parquet` (`set_no`+`game_no` ordering), map to ball-age-cycle position via the 7-then-9 rule, bucket points into "fresh" (games 1-2 post-change) vs "worn" (last 1-2 games pre-change) at the SERVER's own point rows (`point_server`-scoped ace/speed columns); Welch t-test on ace rate and on `speed_kmh`; declared bar |eff|>=0.01 (ace rate points) or |eff|>=1.0 km/h AND p<0.01 (either metric), split-half by match index.
