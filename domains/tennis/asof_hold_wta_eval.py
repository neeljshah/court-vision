"""domains.tennis.asof_hold_wta_eval -- CLI evaluation harness for asof_hold_wta.py.

Runs build_asof_hold_wta and prints:
...
Mirrors domains/tennis/asof_hold_eval.py exactly, sourced from the WTA spine.
...
def run_eval(
    ...
    wta_matches_path: str = _WTA_MATCHES_DEFAULT,
    out_path: str = _OUT_DEFAULT,
    min_prior: int = 5,
) -> None:
    """Build asof_hold_wta parquet and print evaluation metrics."""
    ms = pd.read_parquet(match_stats_path)
    wm = pd.read_parquet(wta_matches_path)
    dest = build_asof_hold_wta(match_stats=ms, wta_matches=wm, out_path=out_path)
    df = pd.read_parquet(dest)

    # ---- Coverage ----
    cov = ((df["p1_n_prior"] >= min_prior) & (df["p2_n_prior"] >= min_prior)).mean()
    print(f"asof_hold_wta: {len(df)} rows -> {dest}")
    print(f"coverage (both >={min_prior} prior matches): {cov:.1%}")

    # ---- Realized stats for comparison ----
    ms_wta = _filter_wta_stats(ms)
...
def _cli() -> None:
    parser = argparse.ArgumentParser(description="Evaluate asof_hold_wta feature quality")
