"""domains.baseball_npb.ratings -- leak-free walk-forward Elo ratings for NPB games.

Thin local mirror of domains.basketball_wnba.ratings (itself a thin mirror of
domains.basketball_nba.ratings): SAME replay algorithm (Elo + home-court
advantage + between-season mean-regression, snapshot-before-update),
parameterized by domains.baseball_npb.elo_config. Duplicated rather than
imported for the same reason as the WNBA module: the NBA/MLB engines hardcode
their own constants at import time (no injection seam), and domains.mlb is not
a sport-blind-importable module -- see the lane's F5 self-containment rule.

TIES (NPB allows draws): a tied game has home_win=NaN. NaN-labeled rows are
EXCLUDED from the Elo update (no winner signal to learn from) but their
PRE-GAME ratings are still recorded in the walk-forward output (so a tie never
silently vanishes from the row count -- it just contributes p_home_elo/
elo_home/elo_away like any other row, with home_win left NaN for the
downstream gate/adapter to drop from any win-prob target).

Input DataFrame columns (from ingest_npb.ingest_seasons):
  date, season, home_team, away_team, home_score, away_score, home_win
  (1.0/0.0/NaN for a tie), tied (bool). Only date, season, home_team,
  away_team, home_win are consumed here; extra columns are preserved in the
  output.

PRIVATE: outputs are price-bearing when combined with odds; data/domains/npb/
is never tracked. No src.*/kernel.*/other-domain imports (F5 compliance).

Per-file test:
  cd /c/Users/neelj/nba-ai-system && python -m pytest domains/baseball_npb/test_ratings.py -q
"""
...
from domains.baseball_npb.elo_config import ELO_K, ELO_MEAN, ELO_HFA, SEASON_REGRESS
...
def _is_tied_row(home_win) -> bool:
    """True iff the row's home_win label is NaN (a recorded NPB draw)."""
    try:
