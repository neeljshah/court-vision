"""Per-file tests for domains.baseball_npb.ratings (pure, offline, synthetic data).
...
from domains.baseball_npb import ratings as r
from domains.baseball_npb.elo_config import ELO_MEAN
...
def test_walk_forward_elo_unseen_teams_start_at_prior():
    df = _games_df([
        (dt.date(2023, 4, 3), 2023, "Giants", "Tigers", 1.0),
    ])
...
def test_walk_forward_elo_winner_rating_increases():
    df = _games_df([
        (dt.date(2023, 4, 3), 2023, "Giants", "Tigers", 1.0),
        (dt.date(2023, 4, 5), 2023, "Giants", "Tigers", 1.0),
    ])
    out = r.walk_forward_elo(df)
...
def test_tied_game_does_not_update_elo_but_is_kept_in_output():
    df = _games_df([
        (dt.date(2023, 4, 3), 2023, "Giants", "Tigers", np.nan),
        (dt.date(2023, 4, 5), 2023, "Giants", "Tigers", 1.0),
    ])
    out = r.walk_forward_elo(df)
    assert len(out) == 2
...
def test_replay_counts_tied_rows_separately_from_decisive():
    df = _games_df([
        (dt.date(2023, 4, 3), 2023, "Giants", "Tigers", np.nan),
        (dt.date(2023, 4, 5), 2023, "Giants", "Tigers", 1.0),
        (dt.date(2023, 4, 7), 2023, "Giants", "Tigers", 0.0),
    ])
    state = r.replay(df)
    assert state.n_processed == 3
    assert state.n_tied == 1
    assert state.counts["Giants"] == 2  # only the two decisive games counted
...
def test_elo_state_asof_matches_fresh_replay_on_subset():
    ...
    df = _games_df([
        (dt.date(2023, 4, 3), 2023, "Giants", "Tigers", 1.0),
        (dt.date(2023, 4, 5), 2023, "Tigers", "Giants", 0.0),
        (dt.date(2023, 4, 8), 2023, "Giants", "Tigers", 1.0),
    ])
...
def test_no_leak_shuffling_future_games_does_not_change_past_ratings():
    ...
    base_rows = [
        (dt.date(2023, 4, 3), 2023, "Giants", "Tigers", 1.0),
        (dt.date(2023, 4, 5), 2023, "Tigers", "Carp", 1.0),
        (dt.date(2023, 4, 8), 2023, "Carp", "Giants", 0.0),
    ]
    future_rows = [
        (dt.date(2023, 5, 1), 2023, "Giants", "Carp", 1.0),
        (dt.date(2023, 5, 3), 2023, "Tigers", "Giants", 0.0),
        (dt.date(2023, 5, 5), 2023, "Carp", "Tigers", 1.0),
    ]
...
def test_season_boundary_regression_pulls_toward_mean():
    df = _games_df([
        ...
        (dt.date(2023, 4, 5), 2023, "Giants", "Tigers", 1.0),
        (dt.date(2023, 4, 8), 2023, "Giants", "Tigers", 1.0),
        (dt.date(2024, 4, 3), 2024, "Giants", "Tigers", 1.0),
    ])
    out = r.walk_forward_elo(df)
