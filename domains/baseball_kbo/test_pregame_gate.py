"""Per-file tests for domains.baseball_kbo.pregame_gate (synthetic data, offline).
...
from domains.baseball_kbo import pregame_gate as pg


def _synthetic_corpus(n_per_season=80, seed=0, tie_rate=0.02):
    """Two synthetic seasons where a persistent-strength signal exists, plus a
    realistic small tie rate (KBO draws), so Elo SHOULD beat coin-flip and
    home-prior baselines on the decisive games."""
    rng = np.random.RandomState(seed)
    teams = ["KIA", "LG", "DOOSAN", "SSG", "SAMSUNG", "KIWOOM"]
...
def test_gate_pregame_reports_tie_exclusion_honestly():
    df = _synthetic_corpus(n_per_season=150, tie_rate=0.04, seed=1)
