"""domains.baseball_kbo.adapter -- KBOAdapter: PREGAME-ONLY two-way moneyline adapter.

Gate seam: feature_bundle() -> FeatureBundle so src.loop.gate.evaluate runs on
KBO data with ZERO kernel edits, mirroring domains.baseball_npb.adapter.
NPBAdapter (same minimal feature_bundle surface, per lane spec).

PREGAME-ONLY this wave (no in-game predict_live -- KBO has no live PBP/base-
out-state ingest built yet; genuinely out of scope for this lane, which is
pregame-Elo-only per the ADAPTER build spec). baseline_probability (pregame
Elo) is the full, real, leak-free signal this adapter offers.
...
F5: imports ONLY stdlib, numpy, pandas, domains.baseball_kbo.*,
...
SPORT_ID = "baseball_kbo"
HOME_SIDE, AWAY_SIDE = "HOME", "AWAY"
GAMES_PARQUET = "data/domains/kbo/kbo_results.parquet"


class KBOAdapter:
    """PREGAME-ONLY KBO two-way moneyline adapter (no in-game predict_live)."""
    ...
    def _get_games(self) -> pd.DataFrame:
        ...
        if not path.exists():
            raise FileNotFoundError(f"kbo_results.parquet not found at {path}.")
    ...
    def _event_id(self, row: pd.Series) -> str:
        """Synthetic event_id: koreabaseball.com's schedule response has no
        numeric game id captured by this scraper, so (date, home_team,
        away_team) is the stable natural key -- the SAME key
        ingest_kbo.ingest_season dedups on."""
    ...
    def market_snapshot(
        ...
        exists yet for KBO (KXKBOGAME/SPREAD/TOTAL/RFI are all live on Kalshi
        NOW per this lane's probe, but capture only starts once a capture
        daemon is wired to them) -- honestly PENDING, never faked. See
        pregame_gate_verdict.json."""
        return None
    ...
    def feature_bundle(
        ...
        *,
        ...
        yet for KBO -- honestly absent, see market_snapshot docstring).
        """
        ...
        if not rows_base:
            raise ValueError(
                ...
                "Check that kbo_results.parquet covers those filters."
            )
