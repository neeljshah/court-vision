"""domains.baseball_kbo.pregame_gate -- honest leak-free KBO pregame calibration gate.

Question: is the walk-forward Elo predictor (domains.baseball_kbo.ratings)
...
TIES EXCLUDED FROM SCORING: KBO allows draws (home_win=NaN). A tied row still
gets a walk-forward pre-game Elo prediction (see ratings.py) but is dropped
from the Brier/logloss/ECE arithmetic here -- there is no {0,1} label to
score against. The exclusion rate is reported per corpus so it is never
silently hidden.

NO CLOSE-LINE COMPARISON YET: no historical Kalshi close-price capture exists
for KBO (KXKBOGAME/SPREAD/TOTAL/RFI are all live NOW per this lane's probe,
but capture only starts once a capture daemon is wired to them). The verdict
is therefore explicitly CALIBRATION_BASELINE_OK / PARTIAL / REJECT /
INSUFFICIENT_DATA -- NEVER a "beats the market" claim. See
.claude/rules/no-edge-claims.md.

Mirrors domains.baseball_npb.pregame_gate structure/thresholds exactly (same
BSS_MIN degenerate-base guard, same N_ECE_BINS, same _decide() logic) -- the
gate math itself is sport-blind; only the tie-exclusion step is KBO-specific.

Pure numpy/pandas + local helpers. No src.*/kernel.* imports.

Per-file test:
  cd /c/Users/neelj/nba-ai-system && python -m pytest domains/baseball_kbo/test_pregame_gate.py -q
"""
...
def gate_pregame(games_df: pd.DataFrame, corpora: Dict[str, List[str]]) -> GateVerdict:
    ...
    cav = [
        ...
        "KBO draws (home_win=NaN) are EXCLUDED from Brier/logloss/ECE scoring "
        "in every corpus -- see n_ties_excluded/tie_rate per corpus.",
        "No historical Kalshi close-price capture exists yet for KBO -- "
